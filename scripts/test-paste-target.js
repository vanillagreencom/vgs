...
test("a terminal app id pastes with Ctrl+Shift+V", () => {
...
test("any other app id, or none, pastes with Ctrl+V", () => {
...
// distinctive names resolve from the last segment; substrings never match.
test("isTerminalAppId matches generic names whole, distinctive names by segment, and no substring", () => {
    for (const [appId, expected, why] of [
        ["com.example.Console", false, "a generic last segment under another vendor is not a terminal"],
        ["com.example.Terminal", false, "a generic last segment under another vendor is not a terminal"],
        ["com.example.Warp", false, "a generic last segment under another vendor is not a terminal"],
        ["com.example.foot", false, "a generic last segment under another vendor is not a terminal"],
        ["com.example.hyper", false, "a generic last segment under another vendor is not a terminal"],
        ["com.example.st", false, "a generic last segment under another vendor is not a terminal"],
        ["org.example.contour", false, "a generic last segment under another vendor is not a terminal"],
        ["org.example.rio", false, "a generic last segment under another vendor is not a terminal"],
        ["net.example.tabby", false, "a generic last segment under another vendor is not a terminal"],
        ["console", false, "a bare console app id is not a terminal"],
        ["terminal", false, "a bare terminal app id is not a terminal"],
        ["warp", false, "a bare warp app id is not a terminal"],
        ["hyper", true, "the hyper terminal's own app id still resolves"],
        ["co.zeit.hyper", false, "co.zeit.hyper is a package name, not a listed app id"],
        ["kitty-notes", false, "a name containing a terminal name is not a terminal"],
        ["fastmail", false, "a name containing st is not a terminal"],
        ["com.example.kitty", true, "a distinctive name resolves from the last segment"]
    ]) {
        assert.equal(PasteTarget.isTerminalAppId(appId), expected, `${appId}: ${why}`);
    }
});

test("normalizeAppId lowers case and strips padding and .desktop, but never to empty", () => {
    for (const [appId, expected, why] of [
        ["  Foot.desktop  ", "foot", "app ids normalize case, padding and .desktop"],
        [".desktop", ".desktop", "a bare .desktop id is not stripped to empty"]
    ]) {
        assert.equal(PasteTarget.normalizeAppId(appId), expected, why);
    }
});

// Client-supplied app IDs must not introduce terminal escapes or extra log lines.
test("displayAppId strips controls, direction marks and line breaks, and clamps the length", () => {
    for (const [appId, expected, why] of [
        ["foot", "foot", "an ordinary app id is logged as-is"],
        ["foo\u001b[31mbar", "foo[31mbar", "escape characters are stripped"],
        ["foo\nWARN forged line", "fooWARN forged line", "newlines cannot forge a log line"],
        ["\u0000a\u007fb\u0080c\u009fd", "abcd", "NUL, DEL and C1 controls are stripped"],
        ["\u0007\u001b", "", "an id of nothing but controls comes back empty"],
        ["", "", "an empty id stays empty"],
        [undefined, "", "a missing id is not the string undefined"],
        ["\u202eDEROLOC", "DEROLOC", "a right-to-left override cannot reorder the line"],
        ["\u2066a\u2069b\u200bc", "abc", "directional isolates and zero-width spaces are stripped"],
        ["a\u2028b\u2029c", "abc", "Unicode line terminators cannot forge a line in a JS log viewer"],
        ["x".repeat(63), "x".repeat(63), "an id below the limit is not truncated"],
        ["x".repeat(64), "x".repeat(64), "an id exactly at the 64-character limit is not truncated"],
        ["x".repeat(65), "x".repeat(64) + "...", "the first id over the limit is clamped and marked"],
        ["x".repeat(500), "x".repeat(64) + "...", "a long id is clamped and marked"]
    ]) {
        assert.equal(PasteTarget.displayAppId(appId), expected, why);
    }
});

// Test both ends of each stripped character range.
test("displayAppId strips both ends of every stripped code point range", () => {
...
test("displayAppId strips the whole Unicode format category", () => {
...
test("the release command releases both modifiers and presses nothing", () => {
...
test("the terminal lists are normalized, sorted, unique and disjoint", () => {
