...
def test_preview_stage_retires_its_window_rule():
    ...
    with patch.object(helper, "run", fake_run), \
        ...
        with helper.preview_stage() as (staged, _reassert):
            ...
            assert_equal(register in calls, True, "the stage registers its window rule")
    ...
    assert_equal(retire in teardown, True, "the teardown must retire its window rule")
    assert_equal(remove in teardown, True, "the teardown must remove the staging output")
