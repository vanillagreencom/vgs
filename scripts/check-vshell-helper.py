...
def test_preview_stage_retires_its_window_rule():
    ...
    for label, session, create_reply, stop_at, staged_expected, retirement, refusals_expected in (
        ...
        ("a compositor with no headless backend, which refuses the output at exit 0", "lua",
         (0, NO_BACKEND), None, False, retire, []),
...
def upstream_port_findings(package: Path, pins: dict) -> tuple[list[str], list[str]]:
    ...
    answers nowhere in its own files, or answers only with a value colors.toml
    may not hold. `terminal-colors.toml` is checked against the slots
