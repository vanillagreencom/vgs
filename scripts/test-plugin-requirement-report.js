...
test("_withheldOnRequirement reports only a recorded, still-unmet refusal of a non-bundled package after the version landed", () => {
    for (const [meta, shell, expected, why] of [
        [{ source: "user", refusedOnRequirement: IMPOSSIBLE, demoted: true }, SHELL, true, "a package refused on the requirement is what this reports"],
        [{ source: "system", refusedOnRequirement: IMPOSSIBLE }, SHELL, true,
            "a refusal that could NOT demote is still a refusal: there was no shipped package to fall back to, so nothing outside pluginLoadErrors reported it before"],
        [{ source: "user", requiresShell: IMPOSSIBLE, demoted: true }, SHELL, false,
            "demoted with an unmet constraint but no recorded refusal is a demotion with ANOTHER cause — a failed startupCheck during the async " +
            "window before the shell version landed, when the version branch is skipped entirely. Blaming the version there misattributes the refusal"],
        [{ source: "user", requiresShell: IMPOSSIBLE }, SHELL, false,
            "a package that still owns its id was never refused: the requirement is not enforced by runStartupGate/loadPlugin/reloadPlugin, so " +
            "reporting it would label a loaded, working plugin as blocked"],
        [{ source: "bundled", refusedOnRequirement: IMPOSSIBLE, demoted: true }, SHELL, false,
            "a bundled id is always-available, so its unmet constraint is inert — reporting it would call a loaded, working package unavailable"],
        [{ source: "user", refusedOnRequirement: SATISFIED, demoted: true }, SHELL, false,
            "a recorded refusal whose constraint this shell now satisfies is stale — the shell was upgraded under it; reporting would send the reader after a solved problem"],
        [{ source: "user", demoted: true }, SHELL, false, "a demotion with nothing recorded has nothing to report as a version refusal"],
        [{ source: "user", refusedOnRequirement: IMPOSSIBLE, demoted: true }, "", false,
            "shell version detection is asynchronous; before it lands an unresolved version parses as 0.0.0 and fails every '>=', so reporting then would be a lie that clears itself"]
    ]) {
...
test("no shipped plugin manifest reports as refused", () => {
