...
test("one user action costs one refresh of each kind", () => {
    for (const [action, names, expected, why] of [
        ...
        ["monitor hotplug that moves focus", ["monitoradded", "focusedmonv2", "activewindowv2"],
            ["refreshMonitors", "toplevelsChanged"],
            "two monitor-list events in one action still cost one j/monitors: the monitor timer collapses them"],
