...
test("PluginComponent ships the hover opt-in off, an empty origin, and gates the hover action on the opt-in", () => {
...
test("pill actions are invoked only through _runPillAction", () => {
    const SAFE_AFTER = /^[,)&|;:]/;
    const directCalls = [];
...
test("_runPillAction fails closed to a non-click origin and restores the previous one", () => {
...
test("exactly the two pills' click handlers claim a click origin", () => {
...
test("no bundled plugin opts into hover actions", () => {
    assert.deepEqual(
        [...optIns].sort(),
        OPTED_IN.slice().sort(),
...
test("pillActionFor acts only on a click origin, cancelling a countdown before stopping a recording", () => {
    for (const [origin, countdown, recording, expected, why] of [
        ["hover", false, true, "ignore", "hovering while recording must not stop the recording"],
        ["hover", true, false, "ignore", "hovering during the countdown must not cancel it"],
        ["click", false, true, "stop", "clicking while recording must stop it"],
        ["click", true, false, "cancel", "clicking during the countdown must cancel it"],
        ["click", false, false, "chooser", "clicking while idle opens the chooser"],
        ["click", true, true, "cancel", "a countdown must be cancelled before a recording is stopped"],
        ["ipc", false, false, "chooser", "an explicit IPC widget toggle may still open the chooser"],
        ["", false, true, "ignore", "an invocation that never set an origin must not be able to stop a recording"],
        ["", true, false, "ignore", "nor cancel a countdown"],
        ["ipc", false, true, "ignore", "an IPC origin must not stop a recording"],
        ["ipc", true, false, "ignore", "nor cancel a countdown"],
        [undefined, false, true, "ignore", "an omitted origin must not stop a recording"],
        [undefined, true, false, "ignore", "nor cancel a countdown"],
        [null, false, true, "ignore", "a null origin must not stop a recording"],
        [null, true, false, "ignore", "nor cancel a countdown"],
        ["Click", false, true, "ignore", "the origin is case-sensitive"],
        ["Click", true, false, "ignore", "the origin is case-sensitive"],
        ["CLICK", false, true, "ignore", "the origin is case-sensitive"],
        ["CLICK", true, false, "ignore", "the origin is case-sensitive"],
        ["hover ", false, true, "ignore", "a padded origin is not a click"],
        ["hover ", true, false, "ignore", "a padded origin is not a click"]
    ]) {
        assert.equal(pillActionFor(origin, countdown, recording), expected,
            `origin ${JSON.stringify(origin)} countdown=${countdown} recording=${recording}: ${why}`);
    }
});
...
test("respondsToHover is true for an opted-in action pill or a popout, and false otherwise", () => {
    for (const [action, optIn, popout, expected, why] of [
        [() => {}, false, false, false, "a widget with a pill action and no hover opt-in and no popout does nothing on hover"],
        [() => {}, true, false, true, "a widget that opted into hover-activation does respond to hover"],
        [null, false, true, true, "a popout widget must keep responding to hover"],
        [() => {}, false, true, true, "an opted-out action pill that also has a popout still opens the popout on hover"],
        [null, false, false, false, "a widget with neither an opt-in nor a popout does nothing on hover"]
    ]) {
        assert.equal(respondsToHover(action, optIn, popout), expected, why);
    }
});
...
test("the bar reads respondsToHover, not the method's presence", () => {
...
test("an ineligible widget is still watched for a capability change", () => {
...
test("an eligible widget becomes a candidate", () => {
...
test("both watch lists include respondsToHoverChanged", () => {
...
test("watcher de-duplication is per object and signal", () => {
...
test("the host-discovery path watches the capability before it filters", () => {
