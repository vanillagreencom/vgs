...
rows=(
  ...
  'env -S sh with an option~vshell-a~#!/usr/bin/env -S sh -e~hyprctl dispatch "hl.dsp.exit()"~1~hyprctl-dispatch-unchecked bin/vshell-a:3'
  'env -S glued~vshell-a~#!/usr/bin/env -Ssh -e~hyprctl dispatch "hl.dsp.exit()"~1~hyprctl-dispatch-unchecked bin/vshell-a:3'
  'env long split-string~vshell-a~#!/usr/bin/env --split-string=sh -e~hyprctl dispatch "hl.dsp.exit()"~1~hyprctl-dispatch-unchecked bin/vshell-a:3'
  'env assignment before the shell~vshell-a~#!/usr/bin/env -S FOO=1 sh -e~hyprctl dispatch "hl.dsp.exit()"~1~hyprctl-dispatch-unchecked bin/vshell-a:3'
  'python interpreter~vshell-py~#!/usr/bin/env python3~subprocess.run(["hyprctl", "dispatch", "exit"])~0~'
