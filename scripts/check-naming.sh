...
scan_owned_source() {
  ...
  if (( status != 1 )); then
    printf 'check-naming: source scan failed with exit %s.\n' "$status" >&2
    return "$status"
  fi
}

scan_curated_vscode_names() {
  local themes_root="$1"
  local curated_name_pattern output status
  curated_name_pattern="\"name\"[[:space:]]*:[[:space:]]*\"[^\"]*(${pattern})"
  if output="$(rg -n -S --glob '**/apps/vscode-theme.json' "$curated_name_pattern" "$themes_root")"
  then
    printf '%s' "$output"
    return 0
  else
  ...
  if (( status != 1 )); then
    printf 'check-naming: curated VS Code name scan failed with exit %s.\n' "$status" >&2
    return "$status"
  fi
}
...
# Curated app files can reference third-party plugins by name. Their VS Code
# display-name field is VGS-owned and must not keep a legacy theme name.
...
if raw="$(scan_owned_source "${paths[@]}")"; then
  :
else
  status=$?
  exit "$status"
fi
if curated_names="$(scan_curated_vscode_names themes)"; then
  :
else
  status=$?
  exit "$status"
fi
if [[ -n "$curated_names" ]]; then
  raw+=$'\n'"$curated_names"
fi
