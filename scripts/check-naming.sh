...
curated_name_pattern="^[[:space:]]*\"name\"[[:space:]]*:[[:space:]]*\"[^\"]*(${pattern})"
if curated_names="$(rg -n -S --glob '**/apps/vscode-theme.json' "$curated_name_pattern" themes)"; then
  raw+=$'\n'"$curated_names"
else
  status=$?
  if (( status != 1 )); then
