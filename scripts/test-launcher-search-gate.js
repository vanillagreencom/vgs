...
const backend = evaluateMarked(serviceSource, "SEARCH BACKEND DECISION", [
    ...
    "dispatchAllowed", "helperHasFallback", "serviceRefuses", "backendStateFrom",
    "canDispatchFrom"
], "DSearchService.qml");
...
// A search the gate refused for want of an answer must not read as one that ran
// and found nothing, and the prompt comes first: nothing is being checked on the
// user's behalf before a query that could dispatch.
assert.equal(view.fileEmptyStateKey("unknown", "", 5, true), "unchecked");
assert.equal(view.fileEmptyStateKey("unknown", "", 5, false), "empty",
    "a search that DID run and found nothing says so");
assert.equal(view.fileEmptyStateKey("checking", "", 0, false), "prompt",
    "an empty field prompts even while the probe is outstanding");
assert.equal(view.fileEmptyStateKey("checking", "", 1, false), "short",
    "and one character is short, not checking");
assert.equal(view.fileEmptyStateKey("checking", "", 2, false), "checking",
    "checking still wins for a query that would actually dispatch");
assert.equal(view.fileHintKey(true, "unknown", "", "retrying"), "probe-failed",
    "the reason shows while the retries run, not only after the last one");

// The leading two arguments must not be interchangeable, or swapping them at the
// call site is invisible: a state in the command slot and a command in the state
// slot have to produce different answers.
assert.notEqual(view.fileEmptyStateKey("missing", "rg", 5, false),
    view.fileEmptyStateKey("rg", "missing", 5, false),
    "fileEmptyStateKey's state and command slots must not be symmetric");
assert.notEqual(view.fileHintKey(true, "missing", "rg", "ready"),
    view.fileHintKey(true, "rg", "missing", "ready"),
    "nor fileHintKey's");
...
assert.ok(files.shouldRetryAfterProbe(true, "notes"),
    "an open surface with a dispatchable query re-runs when the probe answers");
assert.ok(!files.shouldRetryAfterProbe(false, "notes"),
    "a closed one does not — the retry must not search behind a dismissed launcher");
assert.ok(!files.shouldRetryAfterProbe(true, "n"), "nor for a query too short to dispatch");
assert.ok(!files.shouldRetryAfterProbe(true, ""), "nor for none");

assert.ok(view.fileLegActive("files", 0),
    "files mode is a file surface even before a query is typed");
assert.ok(view.fileLegActive("all", 2), "and so is any mode holding a dispatchable file query");
assert.ok(!view.fileLegActive("plugins", 0), "plugins mode is not");
assert.ok(!view.fileLegActive("apps", 1), "nor is a mode with a query too short to dispatch");

// --- 7c. the composition: which property lands in which slot ----------------
//
// Executed, not assumed. The adapters in the QML are one line each and pinned
// verbatim in section 8; everything they decide is decided here.

assert.equal(backend.backendStateFrom("text", "needle", "ready", false, true), "available",
    "text follows the ripgrep slot");
assert.equal(backend.backendStateFrom("text", "needle", "ready", true, false), "missing",
    "and NOT the fd slot — swapping the two is the original VGS-114 symptom exactly");
assert.equal(backend.backendStateFrom("files", "needle", "ready", true, false), "available",
    "name search follows the fd slot");
assert.equal(backend.backendStateFrom("files", "needle", "ready", false, true), "missing");
assert.equal(backend.backendStateFrom("files", "needle", "retrying", true, true), "unknown",
    "a retry episode answers nothing about the tools, whatever the last flags said");
assert.equal(backend.backendStateFrom("files", "needle", "pending", true, true), "checking");

// The unknown-dispatches rule, composed: it is the whole reason a failed probe
// does not disable search, and it must stay kind-aware.
for (const probeState of ["retrying", "failed"]) {
    assert.ok(backend.canDispatchFrom("text", "needle", probeState, false, false),
        `text search dispatches while the probe is ${probeState}: ripgrep fails fast with a real ` +
        "cause, so refusing it would be a silence built on nobody's answer");
    assert.ok(backend.canDispatchFrom("folders", "~/dev", probeState, false, false),
        "and so does path completion, which is one iterdir in the helper");
    for (const kind of ["files", "folders", "all"]) {
        assert.ok(!backend.canDispatchFrom(kind, "needle", probeState, false, false),
            `${kind} does NOT: dispatching it unproven buys the helper's full directory walk, ` +
            "which is the cost the fd gate exists to avoid");
    }
}
// The whole matrix, composed: four flag combinations across every probe state,
// so no cell of it can change without a red.
for (const [fd, rg] of [[true, true], [true, false], [false, true], [false, false]]) {
    for (const probeState of ["pending", "retrying", "failed", "ready"]) {
        const settled = probeState === "ready";
        assert.equal(backend.canDispatchFrom("files", "needle", probeState, fd, rg), settled && fd,
            `a name search dispatches only on a positively detected fd (state=${probeState}, ` +
            `fd=${fd}, rg=${rg})`);
        assert.equal(backend.canDispatchFrom("text", "needle", probeState, fd, rg),
            settled ? rg : probeState !== "pending",
            `text dispatches on ripgrep when known, and on anything past the first probe when ` +
            `not (state=${probeState}, fd=${fd}, rg=${rg})`);
        assert.equal(backend.canDispatchFrom("folders", "~/dev", probeState, fd, rg), true,
...

{
    ...
    q.requires(q.body("backendState"), "backendState()", [
        ["return backendStateFrom(kind, query, statusState, fdAvailable, ripgrepAvailable);",
            "fd in the fd slot and ripgrep in the ripgrep slot — swapping them reinstates " +
            "VGS-114 exactly: ripgrep installed, text search reported missing"]
    ]);
...

{
    ...
    q.requires(q.body("fileSearchQuery"), "fileSearchQuery()", [
        ["return fileSearchQueryFrom(searchMode, searchQuery, prefixInfo ? prefixInfo.query : searchQuery.substring(1).trim());",
            "the mode, the raw query and the PARSED prefix, each in its own slot — this is the " +
            "one authority every other predicate reads"]
    ]);
    ...
    q.requires(q.body("_retryFileSearchAfterProbe"), "_retryFileSearchAfterProbe()", [
        ["if (!shouldRetryAfterProbe(active, fileSearchQuery()))",
            "the retry predicate is the executed one, reading the live active flag and the one " +
            "authority — inverted or dropped, a query typed before the answer never re-runs"],
...

{
    ...
    // The empty state's three bindings, each with its full argument list: the
    // state, the command, the length and the declined flag all have to reach
    // the slot they are read from.
    for (const [call, why] of [
        ["fileEmptyStateKey(_fileBackendState, _missingBackendCommand, _fileQuery.length, _fileSearchDeclined)",
            "the state and the command are not interchangeable, and the declined flag is what " +
            "separates a refused search from one that found nothing"],
        ["root.fileHintKey(root._fileLegActive, root._fileBackendState, root._missingBackendCommand, DSearchService.statusState)",
            "the hint reads the PROBE state in its last slot; fed the backend state instead, the " +
            "probe-failed line never renders and a failed probe goes back to being silent"],
        ["fileLegActive(controller?.searchMode ?? \"\", _fileQuery.length)",
...

{
    ...
    {
        const probe = q.body("_probeStatus");
        const captured = probe.indexOf("const generation = ++_statusGeneration;");
        const guard = probe.indexOf("if (generation !== root._statusGeneration)");
        const inFlightCleared = probe.indexOf("root._statusInFlight = false;");
        const reported = probe.indexOf("root._statusProbeFailed(");
        ...
        assert.ok(probe.indexOf("_statusInFlight = true") < captured,
