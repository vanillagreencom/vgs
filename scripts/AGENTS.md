...
- Every check ships one must-fail control beside it, named `test-<check>`, that plants the defect the check exists to catch.
...
- The memory sampler, the heap-profile attributor and the event-latency benchmark are the measurement tools `docs/architecture/memory.md` describes. A budget quoted anywhere names the tool and the run that produced it.
