...
Item {
    ...
    property string tooltipText: {
        ...
        if (appData.isCoreApp && appData.coreAppData) {
            ...
            appName = Paths.getAppName(appData.appId, effectiveDesktopEntry);
        }
    ...
    function isSpecialWorkspaceShown(specialName) {
        if (!CompositorService.isHyprland || !Hyprland.monitors)
            return false;
        const target = "special:" + specialName;
        const mons = Array.from(Hyprland.monitors.values);
        for (let i = 0; i < mons.length; i++) {
            const sw = String(mons[i].lastIpcObject?.specialWorkspace?.name || "");
            if (sw === target || sw === specialName)
                return true;
        }
        return false;
    }
    ...
    // True when this tile's window(s) live on a Hyprland special workspace (a
    // scratchpad). Drives the corner badge and the real-icon recovery below.
    readonly property bool isScratchpad: {
        if (!appData || !CompositorService.isHyprland || !Hyprland.toplevels)
            return false;
        Hyprland.toplevels.values; // reactive dependency
        if (appData.type === "window") {
            const t = getToplevelObject();
            return !!t && getSpecialWorkspaceName(t) !== "";
        }
        if (appData.type === "grouped") {
            const ws = getGroupedToplevels();
            for (let i = 0; i < ws.length; i++)
                if (getSpecialWorkspaceName(ws[i]) !== "")
                    return true;
        }
        return false;
    }
    ...
    MouseArea {
        ...
        onPressed: mouse => {
            if (mouse.button === Qt.LeftButton && root.canDrag) {
        ...
        onReleased: mouse => {
            ...
            if (didReorder)
                dockApps.moveDockApp(appData.appId, targetIndex);
        ...
        function handleLeftClick() {
            ...
            case "grouped":
                if (appData.windowCount === 0) {
                    ...
                    const shouldHidePin = appData.appId === "org.quickshell" || appData.appId === "com.vanillagreen.vshell";
    ...
    Item {
        ...
        IconImage {
            ...
            anchors.centerIn: parent
            implicitSize: appData && (appData.appId === "org.quickshell" || appData.appId === "com.vanillagreen.vshell") ? actualIconSize * 0.85 : actualIconSize
            source: {
                if (!appData || appData.appId === "__SEPARATOR__") {
                    return "";
                }
                ...
                return Paths.getAppIcon(appData.appId, effectiveDesktopEntry);
            }
            mipmap: true
            smooth: true
            asynchronous: true
            visible: status === Image.Ready && !coreIcon.visible
            layer.enabled: appData && (appData.appId === "org.quickshell" || appData.appId === "com.vanillagreen.vshell")
            layer.smooth: true
            layer.mipmap: true
        ...
        Rectangle {
            ...
            width: Math.max(12, actualIconSize * 0.36)
            height: width
            radius: width / 2
            ...
            anchors.horizontalCenterOffset: actualIconSize / 2 - width * 0.35
            anchors.verticalCenterOffset: -(actualIconSize / 2 - height * 0.35)
