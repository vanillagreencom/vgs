...
Item {
    ...
    function moveDockApp(draggedAppId, toDockIndex) {
        ...
        const current = [...(SessionData.pinnedApps || [])];
        let from = current.indexOf(draggedAppId);
        let to = dockIndexToPinnedIndex(toDockIndex);
        ...
        to = Math.max(0, Math.min(current.length, to));
        current.splice(to, 0, draggedAppId);
        SessionData.setPinnedApps(current);
    }
