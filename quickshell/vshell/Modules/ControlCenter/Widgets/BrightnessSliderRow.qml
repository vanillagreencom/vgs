...
Row {
    ...
    property string targetDeviceName: {
        ...
        const monitor = DisplayService.devices.find(d => d.class === "ddc" || d.class === "apple");
        if (monitor) {
            return monitor.name;
        }
    ...
    Rectangle {
        ...
        // Shift so the glyph's leading edge aligns with the content edge (the trailing
        // slider already reaches the opposite edge); the hover circle bleeds slightly into padding.
        transform: Translate {
            x: -Theme.spacingS
        }
        ...
        MouseArea {
            ...
            onEntered: {
                const tooltipText = targetDevice ? (targetDevice.label || targetDevice.name) : "Brightness Control";
            ...
            VgsIcon {
                ...
                name: {
                    ...
                    if (DisplayService.isDisplayBrightnessClass(targetDevice.class)) {
    ...
    VgsSlider {
        ...
        minimum: {
            ...
            return DisplayService.isDisplayBrightnessClass(targetDevice.class) ? 1 : 0;
        }
        maximum: {
            if (!targetDevice)
                return 100;
