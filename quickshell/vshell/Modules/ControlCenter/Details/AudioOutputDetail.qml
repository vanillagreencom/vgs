...
Rectangle {
    ...
    // Mutes only the notification chime (SettingsData.soundNewNotification);
    // notifications themselves keep arriving. Hidden when the shell has no
    // sound backend (qt6-multimedia missing), since there is nothing to mute.
    VgsToggle {
        id: notifSoundToggle
        anchors.left: parent.left
        anchors.right: parent.right
        ...
        anchors.leftMargin: Theme.spacingM
        anchors.rightMargin: Theme.spacingM
        anchors.topMargin: volumeSlider.visible ? Theme.spacingXS : Theme.spacingS
        visible: AudioService.soundsAvailable
        horizontalPadding: 0
        text: I18n.tr("Notification sounds")
    ...
    VgsFlickable {
        id: audioContent
        anchors.top: notifSoundToggle.visible ? notifSoundToggle.bottom : (volumeSlider.visible ? volumeSlider.bottom : headerRow.bottom)
        anchors.left: parent.left
        anchors.right: parent.right
