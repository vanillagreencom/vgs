...
Item {
    ...
    function sendLockerReadyOnce() {
        ...
        if (SessionService.loginctlAvailable && VGSBackendService.methods.includes("loginctl.lockerReady")) {
            VGSBackendService.sendRequest("loginctl.lockerReady", null, resp => {
                if (resp?.error)
