...
PanelWindow {
    ...
    WindowBlur {
        ...
        blurWidth: !win._finalized ? innerW * s : 0
        blurHeight: !win._finalized ? innerH * s : 0
        blurRadius: Theme.cornerRadius
    }

    WlrLayershell.namespace: "vshell:notification-popup"
    ...
    readonly property bool swipeDismissTowardEdge: {
        if (content.swipeDismissing)
            return _swipeDismissesTowardEdge();
        if (content.swipeActive)
            return content.swipeOffset * _edgeSwipeDirection() > 0;
        return false;
    }
    ...
    readonly property real actionButtonTopMargin: cardPadding + (headerRow.implicitHeight - actionButtonSize) / 2
    readonly property real collapsedContentHeight: Math.max(popupIconSize, textContainer.implicitHeight) + contentBottomClearance
    ...
    function dismissPopupReliably() {
        ...
        // Fallback if wrapperConn.onPopupChanged does not reach startExit.
        Qt.callLater(() => {
    ...
    WlrLayershell.layer: {
        ...
        return LayerShell.fromEnv("VGS_NOTIFICATION_LAYER", fallback);
    }
    ...
    readonly property bool popupWindowShadowActive: Theme.elevationEnabled && SettingsData.notificationPopupShadowEnabled
    ...
    readonly property bool anchorRightEdge: SettingsData.notificationPopupPosition === SettingsData.Position.Top || SettingsData.notificationPopupPosition === SettingsData.Position.Right
    readonly property int windowTopMarginPx: Math.round(getWindowTopMargin())
    readonly property int windowLeftMarginPx: Math.round(getWindowLeftMargin())
    readonly property int windowRightMarginPx: Math.round(getWindowRightMargin())

    anchors.top: true
    anchors.left: !anchorRightEdge
    anchors.bottom: false
    anchors.right: anchorRightEdge
    ...
    margins {
        top: windowTopMarginPx
        bottom: 0
        left: anchorRightEdge ? 0 : windowLeftMarginPx
        right: anchorRightEdge ? windowRightMarginPx : 0
    }
    ...
    function getWindowRightMargin() {
        if (!screen)
            return 0;
        return Theme.snap(getRightMargin() - windowShadowPad, dpr);
    }
    ...
    function _edgeSwipeDirection() {
        const popupPos = SettingsData.notificationPopupPosition;
        return (popupPos === SettingsData.Position.Left || popupPos === SettingsData.Position.Bottom) ? -1 : 1;
    }

    function _swipeDismissesTowardEdge() {
        return content.swipeDismissDirection === _edgeSwipeDirection();
    }
    ...
    Item {
        ...
        ElevationShadow {
            ...
            targetColor: Theme.readableSurface
        ...
        Item {
            ...
            Rectangle {
                ...
                readonly property real surfaceRadius: Theme.cornerRadius
            ...
            StyledText {
                ...
                visible: false
                width: Math.max(0, backgroundContainer.width - Theme.spacingL - win.contentRightInset - popupIconSize - Theme.spacingM)
            ...
            Item {
                ...
                anchors.rightMargin: win.contentRightInset
                height: effectiveCollapsedHeight
            ...
            VgsActionButton {
                ...
                anchors.right: parent.right
                anchors.top: parent.top
                anchors.topMargin: win.actionButtonTopMargin
                anchors.rightMargin: Theme.spacingL
                iconName: "close"
                iconSize: compactMode ? 14 : 16
                buttonSize: win.actionButtonSize
                z: 15
