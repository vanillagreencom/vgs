    ...
    function executeAction(actionIndex) {
        ...
        if (actionIndex >= 0 && actionIndex < actions.length) {
            ...
            if (action && action.invoke) {
                // Release the modal's focus grab BEFORE invoking. When the grab is
                // still held, the target app's xdg-activation request races/loses
                // to the grab and the app only intermittently comes to the front.
                // Close first, then invoke on the next tick once the grab is gone.
                if (onClose)
                    onClose();
