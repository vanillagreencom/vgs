...
PanelWindow {
    id: barWindow
    readonly property var log: Log.scoped("BarWindow")
    ...
    function triggerDashTab(tabIndex) {
        ...
        let section = "center";
        if (clockButtonRef && clockButtonRef.visualContent && dashPopoutLoader.item.setTriggerPosition) {
            const barPosition = axis?.edge === "left" ? 2 : (axis?.edge === "right" ? 3 : (axis?.edge === "top" ? 0 : 1));
            section = clockButtonRef.section || "center";

            const triggerPos = clockButtonRef.visualContent.mapToItem(null, 0, 0);
            const pos = SettingsData.getPopupTriggerPosition(triggerPos, barWindow.screen, barWindow.effectiveBarThickness, clockButtonRef.visualWidth, barConfig?.spacing ?? 4, barPosition, barConfig);
            ...
            dashPopoutLoader.item.triggerScreen = barWindow.screen;
        }

        PopoutManager.requestPopout(dashPopoutLoader.item, tabIndex, (barConfig?.id ?? "default") + "-" + section + "-" + tabIndex);
        return true;
    }
    ...
    readonly property bool usesOverlayLayer: barConfig?.useOverlayLayer ?? false

    readonly property var dBarLayer: LayerShell.fromEnv("VGS_BAR_LAYER", barWindow.usesOverlayLayer ? WlrLayer.Overlay : WlrLayer.Top)
    ...
    Component {
        ...
        // Square minus a quarter-disc: match the curved extension painted by BarCanvas.
        Region {
            id: wingRegion
    ...
    Item {
        ...
        function rebuild() {
            teardown();
            if (!BlurService.backgroundEffectEnabled)
                return;
        ...
        Connections {
            ...
            function onBackgroundEffectEnabledChanged() {
                barBlur.rebuild();
            }
        }
    ...
    readonly property var renderBarConfig: barConfig
    ...
    readonly property bool flattenForMaximizedWindow: true
    ...
    property real effectiveSpacing: (flattenForMaximizedWindow && hasMaximizedToplevel) ? 0 : (barConfig?.spacing ?? 4)
    ...
    readonly property bool effectiveOpenOnOverview: barConfig?.openOnOverview ?? false
    ...
    Connections {
        ...
        function onPluginLoaded(pluginId) {
            log.info("Bar: Plugin loaded:", pluginId);
            SettingsData.widgetDataChanged();
        }
        function onPluginUnloaded(pluginId) {
            log.info("Bar: Plugin unloaded:", pluginId);
            SettingsData.widgetDataChanged();
        }
    }
    ...
    Item {
        ...
        // Mask expansion can emit enter for a stationary pointer in a gap.
        // Reveal only at the thin screen-edge strip while the hidden mask covers the full band.
