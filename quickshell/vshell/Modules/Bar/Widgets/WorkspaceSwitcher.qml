...
import qs.Services
import qs.Widgets
import "../../../Common/WorkspaceSlotPool.js" as WorkspaceSlotPool
...
Item {
    ...
    readonly property string _slotPoolId: WorkspaceSlotPool.createPool()
    ...
    readonly property bool useExtWorkspace: {
        if (Quickshell.env("VSHELL_FORCE_EXTWS") === "1")
    ...
    Connections {
        ...
        function onCompositorChanged() {
            WorkspaceSlotPool.clear(root._slotPoolId);
        }
    }
    ...
    function getSwayActiveWorkspace() {
        ...
        const focusedWs = i3Shim.workspaces?.values?.find(ws => ws.monitor?.name === root.screenName && ws.focused === true);
        return focusedWs ? swayWorkspaceKey(focusedWs) : 1;
    }
    ...
    function getWorkspaceIcons(ws) {
        ...
        wins.forEach((w, i) => {
            ...
            if (!byApp[key]) {
                const isQuickshell = keyBase === "org.quickshell" || keyBase === "com.vanillagreen.vshell";
    ...
    function padWorkspaces(list) {
        ...
        while (padded.length < 3) {
            padded.push(WorkspaceSlotPool.placeholder(root._slotPoolId, slot, () => root._makePlaceholder()));
            slot++;
        }
        return padded;
    }
    ...
    Flow {
        ...
        Repeater {
            ...
            Item {
                ...
                // Square tile side used for the base (no-apps) case in both orientations.
                // Active and inactive share this so the active tile is not wider than the rest.
                readonly property real squareBase: Math.max(root.widgetHeight * 0.7, root.appIconSize * 1.2)
                readonly property real baseWidth: root.isVertical ? (SettingsData.showWorkspaceApps ? Math.max(widgetHeight * 0.7, root.appIconSize + Theme.spacingXS * 2) : squareBase) : squareBase
                readonly property real baseHeight: root.isVertical ? squareBase : (SettingsData.showWorkspaceApps ? Math.max(widgetHeight * 0.7, root.appIconSize + Theme.spacingXS * 2) : squareBase)
                ...
                readonly property real rawVisualWidth: {
                ...
                readonly property real rawVisualHeight: {
                ...
                readonly property real visualWidth: squareTile ? squareSide : rawVisualWidth
                readonly property real visualHeight: squareTile ? squareSide : rawVisualHeight
    ...
    Component.onDestruction: {
        WorkspaceSlotPool.destroy(root._slotPoolId);
