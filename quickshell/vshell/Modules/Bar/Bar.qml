...
Item {
    ...
    Variants {
        ...
        model: {
            ...
            const screens = SettingsData.usableScreens();
            if (prefs.includes("all") || (typeof prefs[0] === "string" && prefs[0] === "all")) {
                return screens;
            }
            const filtered = screens.filter(screen => SettingsData.isScreenInPreferences(screen, prefs));
            if (filtered.length === 0 && root.barConfig?.showOnLastDisplay && screens.length === 1) {
                return screens;
            }
            return filtered;
        }
