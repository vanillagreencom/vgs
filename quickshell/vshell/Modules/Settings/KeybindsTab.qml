...
Item {
    ...
    function confirmRemoveBind(key, remainingKey) {
        removeBindConfirm.showWithOptions({
            ...
            message: KeybindsService.currentProvider === "hyprland" ? I18n.tr("Remove the shortcut %1? An unbind entry will be saved to vgs/binds-user.lua so it stays removed across VGS updates.").arg(key) : I18n.tr("Remove the shortcut %1?").arg(key),
    ...
    function confirmResetBind(key, remainingKey) {
        removeBindConfirm.showWithOptions({
            ...
            message: I18n.tr("Drop your override for %1 so the VGS default action re-applies?").arg(key),
    ...
    VgsFlickable {
        ...
        Column {
            ...
            StyledRect {
                ...
                readonly property var status: KeybindsService.vgsStatus
                    ...
                    Row {
                        ...
                        Column {
                            ...
                            StyledText {
                                text: {
                                    if (warningBox.showLegacy)
                                        return warningBox.status.statusMessage || I18n.tr("VGS can't edit Hyprland shortcuts from Settings; edit your Hyprland config directly.");
                                    if (warningBox.showSetup)
                                        return I18n.tr("Click Setup to create %1 and add the include to your compositor config").arg("vgs/binds");
                                    if (warningBox.showWarning) {
                                        const count = warningBox.status.overriddenBy;
                                        return I18n.ntr("%1 VGS bind may be overridden by config binds that come after the include.", "%1 VGS binds may be overridden by config binds that come after the include.", count).arg(count);
                                    }
                                    return "";
                                }
                                font.pixelSize: Theme.settingsFontSize
            ...
            StyledRect {
                ...
                Column {
                    ...
                    Row {
                        ...
                        StyledText {
                            text: I18n.tr("Loading shortcuts...")
                            font.pixelSize: Theme.fontSizeMedium
                            color: Theme.surfaceVariantText
                            anchors.verticalCenter: parent.verticalCenter
                        }
                    ...
                    StyledText {
                        text: I18n.tr("No shortcuts match the current search or filter")
                        font.pixelSize: Theme.fontSizeMedium
                        color: Theme.surfaceVariantText
                        width: parent.width
