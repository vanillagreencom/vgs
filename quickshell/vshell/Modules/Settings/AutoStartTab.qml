...
Item {
    ...
    FileView {
        ...
        function buildOverrideContent(existing) {
            if (!existing)
                return "[Unit]\nAfter=vshell.service\n";
            const lines = existing.split("\n");
            const hasAfter = lines.some(l => l.trim() === "After=vshell.service");
            if (hasAfter)
                return existing;
            const unitIdx = lines.findIndex(l => l.trim() === "[Unit]");
            if (unitIdx >= 0) {
                lines.splice(unitIdx + 1, 0, "After=vshell.service");
            } else {
                lines.push("[Unit]", "After=vshell.service");
            }
            return lines.join("\n");
        }
    ...
    VgsFlickable {
        ...
        StyledText {
            ...
            text: I18n.tr("Autostart is not available on this system")
        ...
        Column {
            ...
            SettingsCard {
                ...
                SettingsDropdownRow {
                    ...
                    description: I18n.tr("Launch a desktop application or a custom command")
            ...
            SettingsCard {
                ...
                width: parent.width
                iconName: "line_start"
                title: I18n.tr("Entries")
                settingKey: "autostartEntries"
                tags: ["autostart", "startup", "login", "entries"]
                collapsible: true
                expanded: true
                ...
                Row {
                    ...
                    VgsActionButton {
                        ...
                        tooltipText: I18n.tr("Remove All Entries")
                        anchors.verticalCenter: parent.verticalCenter
                        onClicked: {
                            clearAllConfirm.showWithOptions({
                                title: I18n.tr("Remove All Entries?"),
                                message: I18n.tr("Remove all %1 autostart entries? These apps will no longer launch at login.").arg(root.entries.length),
                                confirmText: I18n.tr("Remove All"),
                ...
                Column {
                    ...
                    StyledText {
                        width: parent.width
                        text: I18n.tr("No autostart entries yet — add an application or command above")
                        font.pixelSize: Theme.fontSizeMedium
                        color: Theme.surfaceVariantText
                        horizontalAlignment: Text.AlignHCenter
                        wrapMode: Text.WordWrap
