...
Item {
    ...
    function getAppDisplayName(appId) {
        if (appId === root.vgsChooserId || appId === "vshell-open") {
            return root.vgsChooserLabel;
        }
    ...
    readonly property string vgsChooserId: "vshell-open.desktop"
    readonly property string vgsChooserLabel: I18n.tr("VGS Chooser")

    function withVgsChooser(entries) {
        const filtered = (entries || []).filter(e => e.value !== root.vgsChooserId && e.value !== "vshell-open");
        return [
            {
                text: root.vgsChooserLabel,
                value: root.vgsChooserId
            }
        ].concat(filtered);
    }
    ...
    function loadCategoryModel(categoryKey, categorySearchName) {
        ...
        models[categoryKey] = categoryKey === root.appCategory.Terminal ? entries : root.withVgsChooser(entries);
        root.categoryModels = models;
    }

    function reloadMimeDefaults() {
    ...
    function repairChooserDesktopEntry() {
        Proc.runCommand("desktop-repair-vshell-open", [Paths.vshellCli, "open", "--install-desktop"], (output, exitCode, stderr) => {
            if (exitCode !== 0) {
                ToastService.showError(I18n.tr("Could not install VGS Chooser"), stderr || output);
                return;
            }
            ToastService.showSuccess(I18n.tr("VGS Chooser installed"));
            root.reloadMimeDefaults();
        });
    }
    ...
    Component {
        ...
        Process {
            ...
            command: ["sh", "-c", 'printf "%s.desktop\\n" "$1" > "$2/xdg-terminals.list"', "sh", terminalId, configPath]
    ...
    Connections {
        ...
        function onGetAppsForMimeResult(mimeType, appIds, callbackId) {
            ...
            models[categoryIndex] = root.withVgsChooser(entries);
            root.categoryModels = models;
        }
    ...
    VgsFlickable {
        ...
        Column {
            ...
            SettingsCard {
                title: I18n.tr("VGS Chooser", "default apps chooser card")
                ...
                VgsButton {
                    text: I18n.tr("Install or Repair")
                    iconName: "construction"
                    onClicked: root.repairChooserDesktopEntry()
                }
            }
