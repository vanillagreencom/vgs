...
Singleton {
    ...
    readonly property string greeterAutoLoginPendingSyncPath: "/tmp/vshell-greeter-auto-login-sync-pending"
    ...
    property string vshellU2fPamText: ""
    ...
    readonly property bool lockU2fCustomConfigDetected: pamModuleEnabled(vshellU2fPamText, "pam_u2f")
    ...
    readonly property string greeterFingerprintSource: {
        if (forcedFprintAvailable !== null)
            return forcedFprintAvailable ? "vshell" : "none";
        if (greeterPamHasFprint)
            return "pam";
        switch (fingerprintProbeState) {
        case "ready":
    ...
    readonly property string greeterU2fSource: {
        if (forcedU2fAvailable !== null)
            return forcedU2fAvailable ? "vshell" : "none";
        if (greeterPamHasU2f)
            return "pam";
        if (greeterU2fCanEnable)
            return "vshell";
    ...
    property var greeterAutoLoginSyncProcess: Process {
        command: [Paths.vshellCli, "greeter", "sync", "--yes", "--autologin"]
        running: false
    ...
    property var authApplyProcess: Process {
        command: [Paths.vshellCli, "auth", "sync", "--yes"]
        running: false
    ...
    property var authApplyTerminalFallbackProcess: Process {
        command: [Paths.vshellCli, "auth", "sync", "--terminal", "--yes"]
        running: false
        ...
        onExited: exitCode => {
            if (exitCode === 0) {
                ...
                ToastService.showError(I18n.tr("Terminal fallback failed. Install a supported terminal emulator or run 'vshell auth sync' manually.") + " (exit " + exitCode + ")", details, "", "auth-sync");
            }
            root.finishAuthApply();
        }
    }
    ...
    FileView {
        id: vshellU2fPamWatcher
        path: "/etc/pam.d/vshell-u2f"
        printErrors: false
        onLoaded: root.vshellU2fPamText = text()
        onLoadFailed: root.vshellU2fPamText = ""
    }
