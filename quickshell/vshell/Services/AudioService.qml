...
Singleton {
    ...
    property string wireplumberConfigPath: Paths.strip(StandardPaths.writableLocation(StandardPaths.ConfigLocation)) + "/wireplumber/wireplumber.conf.d/51-vgs-audio-aliases.conf"
    ...
    Connections {
        ...
        function onValuesChanged() {
            ...
            if (root.pendingBtSinkPrefix && root.trySwitchBluetoothSink(root.pendingBtSinkPrefix)) {
                root.pendingBtSinkPrefix = "";
                btSinkSwitchTimer.stop();
            }
        }
    }

    // When a Bluetooth audio device connects, its Pipewire sink node appears
    // asynchronously. Remember which device we want as default output and
    // switch to it as soon as its node shows up (or give up after a while).
    property string pendingBtSinkPrefix: ""

    function switchSinkToBluetoothDevice(address) {
        if (!address)
            return;
        const prefix = "bluez_output." + address.replace(/:/g, "_");
        if (trySwitchBluetoothSink(prefix)) {
            pendingBtSinkPrefix = "";
            btSinkSwitchTimer.stop();
            return;
        }
        pendingBtSinkPrefix = prefix;
        btSinkSwitchTimer.restart();
    }

    function trySwitchBluetoothSink(prefix) {
    ...
    Timer {
        id: btSinkSwitchTimer
        interval: 12000
        repeat: false
        onTriggered: root.pendingBtSinkPrefix = ""
    }
...
`;
    ...
    function generateWireplumberConfig() {
        let config = "# Generated by VGS - Audio Device Aliases\n";
    ...
    function playLoginSoundIfApplicable() {
        if (SettingsData.soundsEnabled && SettingsData.soundLogin && !notificationsAudioMuted) {
            ...
            // to prevent it from playing on every VGS restart during the session
