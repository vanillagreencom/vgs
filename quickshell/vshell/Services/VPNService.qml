...
Singleton {
    ...
    readonly property bool available: NetworkBackendService.vpnAvailable
    ...
    Connections {
        ...
        function onVpnAvailableChanged() {
            if (NetworkBackendService.vpnAvailable && plugins.length === 0) {
                fetchPlugins();
            }
        }
    }
    ...
    function fetchPlugins() {
        ...
        VGSBackendService.sendRequest("network.vpn.plugins", null, response => {
    ...
    function importVpn(filePath, name = "") {
        ...
        VGSBackendService.sendRequest("network.vpn.import", params, response => {
            importing = false;
            ...
            if (response.result.success) {
                ...
                NetworkBackendService.refreshVpnProfiles();
    ...
    function getConfig(uuidOrName) {
        ...
        VGSBackendService.sendRequest("network.vpn.getConfig", {
            uuid: uuidOrName
        }, response => {
            configLoading = false;
    ...
    function updateConfig(uuid, updates) {
        ...
        VGSBackendService.sendRequest("network.vpn.updateConfig", params, response => {
            if (response.error) {
    ...
    function setCredentials(uuid, username, password, save = true) {
        ...
        VGSBackendService.sendRequest("network.vpn.setCredentials", params, response => {
            if (response.error) {
    ...
    function deleteVpn(uuidOrName) {
        if (!available)
            return;
        VGSBackendService.sendRequest("network.vpn.delete", {
            uuid: uuidOrName
        }, response => {
            if (response.error) {
