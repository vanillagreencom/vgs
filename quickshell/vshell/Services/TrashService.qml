...
Singleton {
    ...
    function refreshCount() {
        Proc.runCommand("trash-count", [Paths.vshellCli, "trash", "count"], (output, exitCode) => {
            if (exitCode !== 0) {
    ...
    function trashPath(path, callback) {
        ...
        Proc.runCommand(null, [Paths.vshellCli, "trash", "put", path], (output, exitCode) => {
            const ok = exitCode === 0;
            if (!ok)
    ...
    function emptyTrash() {
        Proc.runCommand("trash-empty", [Paths.vshellCli, "trash", "empty"], (output, exitCode) => {
            if (exitCode !== 0)
