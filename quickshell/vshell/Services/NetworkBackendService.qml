...
Singleton {
    ...
    readonly property bool supportsNetworkPreference: VGSBackendService.methods.includes("network.preference.set")
    ...
    function getState() {
        if (!networkAvailable)
            return;
        VGSBackendService.sendRequest("network.getState", null, response => {
            if (response.result) {
    ...
    function updateState(state) {
        ...
        if (state.wifiNetworks || state.savedWifiNetworks) {
            const hasSavedWifiState = supportsSavedWifiState && Array.isArray(state.savedWifiNetworks);
    ...
    function connectToSpecificWiredConfig(uuid) {
        ...
        VGSBackendService.sendRequest("network.ethernet.connect.config", params, response => {
            if (response.error) {
    ...
    function scanWifi() {
        ...
        VGSBackendService.sendRequest("network.wifi.scan", params, response => {
            isScanning = false;
            if (response.error) {
    ...
    function connectToWifi(ssid, password = "", username = "", anonymousIdentity = "", domainSuffixMatch = "", hidden = false) {
        ...
        VGSBackendService.sendRequest("network.wifi.connect", params, response => {
            if (response.error) {
    ...
    function disconnectWifi() {
        ...
        VGSBackendService.sendRequest("network.wifi.disconnect", params, response => {
            if (response.error) {
    ...
    function submitCredentials(token, secrets, save) {
        log.debug("submitCredentials: networkAvailable=" + networkAvailable + " supportsCredentials=" + supportsCredentials);

        if (!networkAvailable || !supportsCredentials) {
            log.warn("submitCredentials: Aborting - credentials backend unavailable");
            return;
        }
        ...
        VGSBackendService.sendRequest("network.credentials.submit", params, response => {
            if (response.error) {
    ...
    function cancelCredentials(token) {
        ...
        VGSBackendService.sendRequest("network.credentials.cancel", params, response => {
            if (response.error) {
    ...
    function forgetWifiNetwork(ssid) {
        if (!networkAvailable)
            return;
        forgetSSID = ssid;
        VGSBackendService.sendRequest("network.wifi.forget", {
            ssid: ssid
        }, response => {
            if (response.error) {
    ...
    function toggleWifiRadio() {
        if (!networkAvailable || wifiToggling)
            return;
        wifiToggling = true;
        VGSBackendService.sendRequest("network.wifi.toggle", null, response => {
            wifiToggling = false;
    ...
    function enableWifiDevice() {
        if (!networkAvailable)
            return;
        VGSBackendService.sendRequest("network.wifi.enable", null, response => {
            if (response.error) {
    ...
    function setNetworkPreference(preference) {
        ...
        VGSBackendService.sendRequest("network.preference.set", {
    ...
    function toggleNetworkConnection(type) {
        ...
        if (type === "ethernet") {
            if (ethernetConnected) {
                VGSBackendService.sendRequest("network.ethernet.disconnect", null, null);
            } else {
                VGSBackendService.sendRequest("network.ethernet.connect", null, null);
            }
        }
    }
    ...
    function disconnectEthernetDevice(deviceName) {
        if (!networkAvailable)
            return;
        VGSBackendService.sendRequest("network.ethernet.disconnect", {
            device: deviceName
        }, null);
    }
    ...
    function fetchWiredNetworkInfo(uuid) {
        ...
        VGSBackendService.sendRequest("network.ethernet.info", {
            uuid: uuid
        }, response => {
    ...
    function fetchNetworkInfo(ssid) {
        ...
        VGSBackendService.sendRequest("network.info", {
            ssid: ssid
        }, response => {
    ...
    function refreshVpnProfiles() {
        if (!vpnAvailable)
            return;
        VGSBackendService.sendRequest("network.vpn.profiles", null, response => {
            if (response.result) {
    ...
    function refreshVpnActive() {
        if (!vpnAvailable)
            return;
        VGSBackendService.sendRequest("network.vpn.active", null, response => {
            if (response.result) {
    ...
    function connectVpn(uuidOrName, singleActive = false) {
        ...
        VGSBackendService.sendRequest("network.vpn.connect", params, response => {
            if (response.error) {
                vpnIsBusy = false;
                pendingVpnUuid = "";
                vpnBusyStartTime = 0;
    ...
    function disconnectVpn(uuidOrName) {
        ...
        VGSBackendService.sendRequest("network.vpn.disconnect", params, response => {
            if (response.error) {
                vpnIsBusy = false;
                vpnBusyStartTime = 0;
    ...
    function disconnectAllVpns() {
        ...
        VGSBackendService.sendRequest("network.vpn.disconnectAll", null, response => {
            if (response.error) {
                vpnIsBusy = false;
    ...
    function setWifiAutoconnect(ssid, autoconnect) {
        if (!networkAvailable || !supportsWifiAutoconnect)
            return;
        const params = {
            ssid: ssid,
        ...
        VGSBackendService.sendRequest("network.wifi.setAutoconnect", params, response => {
            if (response.error) {
