...
Singleton {
    ...
    property bool vgsBindsIncluded: true

    property var vgsStatus: ({
            "exists": true,
            "included": true,
            "includePosition": -1,
            "totalIncludes": 0,
    ...
    readonly property string vgsBindsPath: {
        switch (currentProvider) {
        case "niri":
            return compositorConfigDir + "/vgs/binds.kdl";
        case "hyprland":
            return compositorConfigDir + "/vgs/binds.lua";
        case "mangowc":
            return compositorConfigDir + "/vgs/binds.conf";
        default:
            return "";
        }
    }
    ...
    Process {
        ...
        onExited: exitCode => {
            ...
            root.lastError = "";
            root.vgsBindsIncluded = true;
            root.vgsBindsFixed();
            const bindsRel = root.currentProvider === "niri" ? "vgs/binds.kdl" : root.currentProvider === "hyprland" ? "vgs/binds.lua" : "vgs/binds.conf";
    ...
    function fixVgsBindsInclude() {
        if (fixing || vgsBindsIncluded || !compositorConfigDir)
            return;
        if (readOnly) {
        ...
        case "hyprland":
            script = ConfigIncludeResolve.buildRepairScript({
                ...
                fragmentFiles: [compositorConfigDir + "/vgs/binds.lua", compositorConfigDir + "/vgs/binds-user.lua"],
                includes: [
                    {
                        grepPattern: "vgs.binds",
                        includeLine: "require(\"vgs.binds\")"
                    },
                    {
                        grepPattern: "vgs.binds-user",
                        includeLine: "require(\"vgs.binds-user\")"
                    }
                ]
            });
            break;
        case "mangowc":
            script = ConfigIncludeResolve.buildRepairScript({
                configFile: mainConfigPath,
                backupFile: backupPath,
                fragmentFile: compositorConfigDir + "/vgs/binds.conf",
                grepPattern: "source.*vgs/binds.conf",
                includeLine: "source = ./vgs/binds.conf"
            });
            break;
        default:
            fixing = false;
    ...
    function loadCheatsheet(provider) {
        ...
        cheatsheetProcess.command = [Paths.vshellCli, "keybinds", "show", target];
    ...
    function loadBinds(showLoading) {
        ...
        loadProcess.command = [Paths.vshellCli, "keybinds", "show", currentProvider];
    ...
    function _processData() {
        ...
        vgsBindsIncluded = _rawData?.vgsBindsIncluded ?? true;
        const status = _rawData?.vgsStatus;
        if (status) {
            vgsStatus = {
                ...
                "bindsAfterVgs": status.bindsAfterVgs ?? 0,
        ...
        for (const cat in bindsData) {
            ...
            for (var i = 0; i < binds.length; i++) {
                const bind = binds[i];
                const targetCat = Actions.isVgsAction(bind.action) ? "VGS" : cat;
        ...
        for (var ci = 0; ci < sortedCats.length; ci++) {
            ...
            for (var i = 0; i < binds.length; i++) {
                ...
                const keyData = {
                    ...
                    "desc": bind.desc || "",
                    "source": sourceStr,
                    "isOverride": sourceStr === "vgs",
                    "isVGSManaged": sourceStr === "vgs" || sourceStr === "vgs-default",
    ...
    function saveBind(originalKey, bindData) {
        ...
        if (!bindData.key || !Actions.isValidAction(bindData.action))
            return;
        saving = true;
        const cmd = [Paths.vshellCli, "keybinds", "set", currentProvider, bindData.key, bindData.action, "--desc", bindData.desc || ""];
    ...
    function _maybeWarnHyprlandLegacyConf() {
        ...
        if (!vgsStatus.exists || vgsStatus.included)
            return;
        _hyprlandLegacyWarnShown = true;
    ...
    function showHyprlandReadOnlyWarning() {
        ToastService.showWarning(I18n.tr("Hyprland edits are read-only"), vgsStatus.statusMessage || I18n.tr("VGS can't edit Hyprland shortcuts from Settings; edit your Hyprland config directly."), "", "hyprland-migration");
    }
    ...
    function removeBind(key) {
        ...
        if (!key)
            return;
        removeProcess.command = [Paths.vshellCli, "keybinds", "remove", currentProvider, key];
    ...
    function resetBind(key) {
        ...
        if (!key)
            return;
        removeProcess.command = [Paths.vshellCli, "keybinds", "reset", currentProvider, key];
        removeProcess.running = true;
        bindRemoved(key);
    }

    function isVgsAction(action) {
        return Actions.isVgsAction(action);
    }
    ...
    function getVgsActions() {
        return Actions.getVgsActions(CompositorService.isNiri, CompositorService.isHyprland);
    }
