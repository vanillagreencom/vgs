...
Singleton {
    ...
    readonly property string _confPath: `${Paths.strip(StandardPaths.writableLocation(StandardPaths.TempLocation))}/vgs-cava-${Date.now()}-${Math.floor(Math.random() * 1000000)}.conf`
    ...
    Process {
        ...
        running: false
        onExited: exitCode => {
            root.cavaAvailable = exitCode === 0 && Quickshell.env("VGS_DISABLE_CAVA") !== "1";
        }
    }
