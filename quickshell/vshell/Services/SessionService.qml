...
Singleton {
    ...
    function launchDesktopEntry(desktopEntry, useNvidia) {
        ...
        const defaultPrefix = Quickshell.env("VGS_DEFAULT_LAUNCH_PREFIX") || "";
    ...
    function enableIdleInhibit() {
        ...
        idleInhibited = true;
        inhibitorChanged();
        ToastService.showInfo(I18n.tr("Idle inhibitor on • staying awake"));
    }
    ...
    function disableIdleInhibit() {
        ...
        idleInhibited = false;
        inhibitorChanged();
        ToastService.showInfo(I18n.tr("Idle inhibitor off • system can sleep"));
    }
    ...
    function ensurePrepareForSleepSubscription() {
        ...
        VGSBackendService.dbusSubscribe("system", "org.freedesktop.login1", "/org/freedesktop/login1", "org.freedesktop.login1.Manager", "PrepareForSleep", response => {
            prepareForSleepSubscriptionPending = false;
    ...
    function getLoginctlState() {
        if (!loginctlAvailable)
            return;
        VGSBackendService.sendRequest("loginctl.getState", null, response => {
            if (response.result) {
    ...
    function syncLockBeforeSuspend() {
        if (!loginctlAvailable)
            return;
        VGSBackendService.sendRequest("loginctl.setLockBeforeSuspend", {
    ...
    function syncSleepInhibitor() {
        if (!loginctlAvailable)
            return;
        if (!VGSBackendService.methods.includes("loginctl.setSleepInhibitorEnabled"))
            return;
        VGSBackendService.sendRequest("loginctl.setSleepInhibitorEnabled", {
