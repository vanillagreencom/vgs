...
Singleton {
    ...
    function pushConfigToServer() {
        if (!serverSchedulingAvailable)
            return;
        VGSBackendService.sendRequest("wallpaper.setConfig", {
    ...
    function cycle(screenName, wallpaperPath, goToPrevious) {
        ...
        if (!currentWallpaper)
            return;
        // A theme's wallpaper set can merge the repo package with the user
        // overlay (two directories), so probe the composed set first and only
        // fall back to scanning the current wallpaper's directory.
        runCycleProcess([Paths.vshellCli, "theme", "wallpapers", "--json"], screenName || "", currentWallpaper, goToPrevious, true);
    }

    function runCycleProcess(command, screenName, currentWallpaper, goToPrevious, probingThemeSet) {
    ...
    function resetScheduleAfterManual() {
        if (!serverSchedulingAvailable)
            return;
        VGSBackendService.sendRequest("wallpaper.trigger", {
            "target": ""
        }, null);
    }
    ...
    function applyCycledWallpaper(text, currentPath, targetScreenName, goToPrevious, probingThemeSet) {
        if (probingThemeSet) {
            let themeSet = [];
            try {
                themeSet = (JSON.parse(text || "{}").wallpapers || []).map(w => w.path);
            } catch (e) {
                themeSet = [];
            }
            if (themeSet.length > 1 && themeSet.indexOf(currentPath) !== -1) {
                // Keep the helper's composed order; do not re-sort merged paths.
                applyFromList(themeSet, currentPath, targetScreenName, goToPrevious);
                return;
            }
            const wallpaperDir = currentPath.substring(0, currentPath.lastIndexOf('/'));
            runCycleProcess(findCommand(wallpaperDir), targetScreenName, currentPath, goToPrevious, false);
            return;
        }
        if (!text || !text.trim())
            return;
        ...
        if (files.length <= 1)
            return;
        applyFromList(files.sort(), currentPath, targetScreenName, goToPrevious);
    }

    function applyFromList(wallpaperList, currentPath, targetScreenName, goToPrevious) {
    ...
    Component {
        ...
        Process {
            ...
            property bool probingThemeSet: false
            running: false
            stdout: StdioCollector {
                onStreamFinished: root.applyCycledWallpaper(text, currentWallpaper, targetScreenName, goToPrevious, probingThemeSet)
            }
        }
    }
    ...
    Process {
        ...
        running: false
        stdout: StdioCollector {
            onStreamFinished: root.applyCycledWallpaper(text, cyclingProcess.currentWallpaper, cyclingProcess.targetScreenName, cyclingProcess.goToPrevious, cyclingProcess.probingThemeSet)
        }
    }
    ...
    Process {
        ...
        running: false
        stdout: StdioCollector {
            onStreamFinished: root.applyCycledWallpaper(text, prevCyclingProcess.currentWallpaper, prevCyclingProcess.targetScreenName, prevCyclingProcess.goToPrevious, prevCyclingProcess.probingThemeSet)
        }
    }
}
