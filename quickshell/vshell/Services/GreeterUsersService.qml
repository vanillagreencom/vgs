...
Singleton {
    ...
    readonly property string greetCfgDir: Quickshell.env("VSHELL_GREET_CFG_DIR") || "/var/cache/vshell-greeter"
    ...
    function syncedThemeCheckScript(username) {
        const dir = userCacheDir(username);
        if (!dir)
            return "echo " + username + ":0";
        const settings = JSON.stringify(dir + "/settings.json");
        const theme = JSON.stringify(dir + "/theme.json");
        const session = JSON.stringify(dir + "/session.json");
        const manifest = JSON.stringify(dir + "/sync-manifest.json");
        return `[ -f ${settings} ] && [ -f ${theme} ] && [ -f ${session} ] && [ -f ${manifest} ] && echo ${JSON.stringify(username + ":1")} || echo ${JSON.stringify(username + ":0")}`;
    }
    ...
    function _refreshSyncedThemeFlags() {
        ...
        const checks = usernames.map(u => syncedThemeCheckScript(u)).join("; ");
