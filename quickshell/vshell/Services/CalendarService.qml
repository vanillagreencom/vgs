...
Singleton {
    ...
    readonly property bool vgsNeedsLaunch: backendPref === "vgs-calendar" && !vgsBackend.connected && !vgsBackend.socketFound

    property var calendars: vgsBackend.calendars
    ...
    CalendarBackend {
        id: vgsBackend
        enabled: root.backendPref === "vgs-calendar" || root.backendPref === "auto"
        onEventsByDateChanged: root.mergeEvents()
        onConnectedChanged: {
    ...
    function loadEvents(startDate, endDate) {
        ...
        case "vgs-calendar":
            vgsBackend.loadEvents(startDate, endDate);
            break;
        case "khal":
    ...
    function _activeBackendEventsByDate() {
        switch (activeBackend) {
        case "vgs-calendar":
            return vgsBackend.eventsByDate;
        case "khal":
    ...
    function writableCalendars() {
        return isVgsActive ? vgsBackend.writableCalendars() : [];
    }

    function defaultCalendar() {
        return isVgsActive ? vgsBackend.defaultCalendar() : null;
    }

    function launchVgsCalendar() {
        vgsBackend.launch();
    }

    function createEvent(fields, callback) {
        if (isVgsActive) {
            vgsBackend.createEvent(fields, callback);
            return;
        }
        if (callback)
            callback({
    ...
    function updateEvent(id, fields, callback) {
        if (isVgsActive) {
            vgsBackend.updateEvent(id, fields, callback);
            return;
        }
        if (callback)
            callback({
    ...
    function deleteEvent(id, callback) {
        if (isVgsActive) {
            vgsBackend.deleteEvent(id, callback);
            return;
        }
        if (callback)
            callback({
    ...
    function addTaskForDate(date, text) {
        ...
        if (!tasks[dateKey])
            tasks[dateKey] = [];
        let taskId = (new Date().getTime()) + "-vgs";
        tasks[dateKey].push({
            "id": taskId,
            "text": text,
            "completed": false
