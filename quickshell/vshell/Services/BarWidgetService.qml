...
Singleton {
    ...
    function triggerWidgetPopout(widgetId) {
        ...
        const signalMap = {
            ...
            "network_speed_monitor": "networkClicked"
        };
    ...
    function getBarWindowForScreen(screenName) {
        ...
        for (var i = 0; i < barRepeater.count; i++) {
            const loader = barRepeater.itemAt(i);
            if (!loader?.item)
                continue;
    ...
    function getFirstBarWindow() {
        if (!barRepeater || barRepeater.count === 0)
            return null;

        const loader = barRepeater.itemAt(0);
        if (!loader?.item)
            return null;
