            ...
            ColumnLayout {
                ...
                Rectangle {
                    ...
                    Column {
                        ...
                        StyledText {
                            text: I18n.tr("To use this VGS bind, remove or change the keybind in your config.kdl")
                            font.pixelSize: Theme.fontSizeSmall
                            color: Theme.surfaceVariantText
                            width: parent.width
                            wrapMode: Text.WordWrap
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                readonly property var tooltipTexts: ({
                                        "vgs": I18n.tr("VGS shell actions (launcher, clipboard, etc.)"),
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        ...
                                        case "vgs":
                                            root.updateEdit({
                                                "action": KeybindsService.vgsActions[0].id,
                                                "desc": KeybindsService.vgsActions[0].label
                                            });
                                            break;
                                        case "compositor":
                                            root.updateEdit({
                ...
                RowLayout {
                    ...
                    VgsDropdown {
                        ...
                        options: KeybindsService.getVgsActions().map(a => a.label)
                        enableFuzzySearch: true
                        maxPopupHeight: 300
                        onValueChanged: value => {
                            if (root.readOnly)
                                return;
                            const actions = KeybindsService.getVgsActions();
                ...
                RowLayout {
                    ...
                    readonly property var parsedArgs: argConfig?.type === "vgs" ? Actions.parseVgsActionArgs(root.editAction) : null
                    readonly property var vgsActionArgs: Actions.getVgsActionArgs()
                    readonly property bool hasAmountArg: parsedArgs?.base ? (vgsActionArgs[parsedArgs.base]?.args?.some(a => a.name === "amount") ?? false) : false
                    readonly property bool hasDeviceArg: parsedArgs?.base ? (vgsActionArgs[parsedArgs.base]?.args?.some(a => a.name === "device") ?? false) : false
                    readonly property bool hasTabArg: parsedArgs?.base ? (vgsActionArgs[parsedArgs.base]?.args?.some(a => a.name === "tab") ?? false) : false

                    visible: root._actionType === "vgs" && argConfig?.type === "vgs"
                    ...
                    StyledText {
                        ...
                        color: Theme.surfaceVariantText
                        Layout.preferredWidth: root._labelWidth
                        visible: vgsArgsRow.hasAmountArg
                    }
                    ...
                    VgsTextField {
                        ...
                        Connections {
                            ...
                            function onParsedArgsChanged() {
                                const newText = vgsArgsRow.parsedArgs?.args?.amount || "";
                                if (vgsAmountField.text !== newText)
                                    vgsAmountField.text = newText;
                            }
                        }

                        Component.onCompleted: {
                            text = vgsArgsRow.parsedArgs?.args?.amount || "";
                        }

                        onEditingFinished: {
                            if (!vgsArgsRow.parsedArgs)
                                return;
                            const oldAction = root.editAction;
                            const newArgs = Object.assign({}, vgsArgsRow.parsedArgs.args);
                            newArgs.amount = text || "5";
                            const newAction = Actions.buildVgsAction(vgsArgsRow.parsedArgs.base, newArgs);
                            const changes = {
                                "action": newAction
                            };
                    ...
                    StyledText {
                        ...
                        font.weight: Font.Medium
                        color: Theme.surfaceVariantText
                        Layout.leftMargin: vgsArgsRow.hasAmountArg ? Theme.spacingM : 0
                        Layout.preferredWidth: vgsArgsRow.hasAmountArg ? -1 : root._labelWidth
                        visible: vgsArgsRow.hasDeviceArg
                    }
                    ...
                    VgsTextField {
                        ...
                        Connections {
                            target: vgsArgsRow
                            function onParsedArgsChanged() {
                                const newText = vgsArgsRow.parsedArgs?.args?.device || "";
                                if (vgsDeviceField.text !== newText)
                                    vgsDeviceField.text = newText;
                            }
                        }

                        Component.onCompleted: {
                            text = vgsArgsRow.parsedArgs?.args?.device || "";
                        }
                        ...
                        onEditingFinished: {
                            ...
                            newArgs.device = text;
                            root.updateEdit({
                                "action": Actions.buildVgsAction(vgsArgsRow.parsedArgs.base, newArgs)
                            });
                        }
                    }
                    ...
                    Item {
                        Layout.fillWidth: true
                        visible: !vgsArgsRow.hasDeviceArg && !vgsArgsRow.hasTabArg
                    }
                    ...
                    VgsDropdown {
                        ...
                        visible: vgsArgsRow.hasTabArg
                        currentValue: {
                            const tab = vgsArgsRow.parsedArgs?.args?.tab || "";
                            switch (tab) {
                            case "media":
                ...
                RowLayout {
                    ...
                    VgsActionButton {
                        ...
                        visible: root.editingKeyIndex >= 0 && root.editingKeyIndex < root.keys.length && (root.keys[root.editingKeyIndex].isVGSManaged || root.keys[root.editingKeyIndex].isOverride) && !root.isNew && !root.readOnly
