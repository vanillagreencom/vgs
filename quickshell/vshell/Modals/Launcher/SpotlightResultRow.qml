...
Rectangle {
    ...
    Column {
        ...
        // maximumLineCount/elide don't cap Text.RichText, so a highlighted (search)
        // name — e.g. a long clipboard preview — wrapped and ran over the fixed-height
        // row. Pre-elide the rich name to a one-line character budget (measured from
        // the plain text), exactly as the subtitle below already does.
        TextMetrics {
            id: nameProbe
            ...
            text: root.item?._hRich ? HtmlElide.stripHtmlTags(root.item?._hName ?? "") : ""
        }

        readonly property int _nameBudget: {
            if (!nameProbe.text)
                return 0;
            const elided = nameProbe.elidedText;
        ...
        StyledText {
            id: nameText
            width: parent.width
            text: root.item?._hRich ? HtmlElide.elideRichText(root.item?._hName ?? "", textColumn._nameBudget) : (root.item?.name ?? "")
