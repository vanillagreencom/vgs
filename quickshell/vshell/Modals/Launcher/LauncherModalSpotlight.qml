...
Item {
    id: root
    readonly property var log: Log.scoped("LauncherModalSpotlight")
    ...
    // Fixed 680px width, centered horizontally.
    readonly property int modalWidth: Math.min(680, screenWidth - 80)
    readonly property real modalX: Math.max(0, (screenWidth - modalWidth) / 2)
    // Keep the search bar centered; results expand downward unless the screen edge clamps it.
    readonly property real modalY: {
        const searchBarH = 56;
        const preferred = Math.max(0, screenHeight * 0.33 - searchBarH / 2);
        const maxY = Math.max(0, screenHeight - 56);
        return Math.max(0, Math.min(preferred, maxY));
    }
