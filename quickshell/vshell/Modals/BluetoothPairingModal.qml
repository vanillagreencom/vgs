...
VgsModal {
    ...
    layerNamespace: "vshell:bluetooth-pairing"
    ...
    onBackgroundClicked: () => {
        if (token) {
            VGSBackendService.bluetoothCancelPairing(token);
        }
        close();
        token = "";
        pinInput = "";
    ...
    function submitPairing() {
        ...
        VGSBackendService.bluetoothSubmitPairing(token, secrets, true, response => {
            if (response.error) {
