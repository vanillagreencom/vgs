...
Item {
    ...
    readonly property bool canOpenEntry: !!entry && (entry.isImage ?? false) && !!entry.path
    ...
    function openEntry() {
        if (!entry)
            return;
        ClipboardService.openEntry(entry);
        hide();
    }
    ...
    PanelWindow {
        ...
        WlrLayershell.namespace: "vshell:clipboard-context-menu"
