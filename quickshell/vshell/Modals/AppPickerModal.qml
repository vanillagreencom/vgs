...
VgsModal {
    ...
    function fetchMimeMatches() {
        ...
        if (queriedMime.length === 0)
            return;
        DesktopService.getAppsForMimeType(queriedMime, "app-picker");
    }
    ...
    Connections {
        ...
        function onGetAppsForMimeResult(resultMimeType, appIds, callbackId) {
            if (callbackId !== "app-picker" || resultMimeType !== root.mimeType)
                return;
            mimeMatchedAppIds = (appIds || []).map(_normAppId);
            updateApplicationList();
        }
    }
