...
Item {
    ...
    readonly property var log: Log.scoped("VGSIPC")

    required property var captureModalLoader
    ...
    required property var barRepeater
    ...
    function getPreferredBar(refPropertyName) {
        ...
        const loaders = Array.from({
            length: root.barRepeater.count
        }, (_, i) => root.barRepeater.itemAt(i));
    ...
    function launchDefaultMimeApp(appName, mimeType) {
        const callbackId = "ipc-" + appName;
        DesktopService.getDefaultApp(mimeType, callbackId);
    ...
    Connections {
        ...
        function onGetDefaultAppResult(mimeType, desktopFileId, callbackId) {
            if (!callbackId || !callbackId.startsWith("ipc-"))
                return;
            root.launchDesktopId(desktopFileId || "", callbackId.slice(4));
        }
    }
    ...
    IpcHandler {
        function countdown(remaining: int, total: int, mode: string): string {
            CaptureService.updateCountdown(remaining, total, mode);
            return "CAPTURE_COUNTDOWN_UPDATED";
        }

        function countdownEnd(): string {
            CaptureService.endCountdown();
            return "CAPTURE_COUNTDOWN_ENDED";
        }

        function open(): string {
            return root.captureModalLoader.request("open") ? "CAPTURE_OPEN_SUCCESS" : "CAPTURE_OPEN_FAILED";
        }

        function openOnScreen(screenName: string): string {
            if (screenName && !Quickshell.screens.some(screen => screen.name === screenName))
                return "CAPTURE_OPEN_FAILED: unknown screen " + screenName;
            return root.captureModalLoader.request("open", screenName) ? "CAPTURE_OPEN_SUCCESS" : "CAPTURE_OPEN_FAILED";
        }

        function close(): string {
            return root.captureModalLoader.request("close") ? "CAPTURE_CLOSE_SUCCESS" : "CAPTURE_NOT_OPEN";
        }

        function toggle(): string {
            return root.captureModalLoader.request("toggle") ? "CAPTURE_TOGGLE_SUCCESS" : "CAPTURE_TOGGLE_FAILED";
        }
    ...
    IpcHandler {
        function _openDashTab(tabId: string, mode: string): string {
            ...
            if (!bar)
                return "THEME_PICKER_UNAVAILABLE";
            root.dashPopoutLoader.active = true;
            const dash = root.dashPopoutLoader.item;
            if (!dash)
                return "THEME_PICKER_UNAVAILABLE";
            dash.themesModeFilter = ["all", "dark", "light"].includes(mode) ? mode : "all";
            const tabIndex = SettingsData.dashTabIndexForId(tabId);
            if (dash.shouldBeVisible && dash.triggerScreen?.name === bar.screen?.name) {
                dash.currentTabIndex = tabIndex;
                if (dash.updateSurfacePosition)
                    dash.updateSurfacePosition();
                return "THEME_PICKER_OPEN_SUCCESS";
            }
        ...
        function open(): string {
            return _openDashTab("themes", "all");
        }

        function openMode(mode: string): string {
            return _openDashTab("themes", mode);
        }

        function openWallpapers(): string {
            return _openDashTab("wallpaper", "all");
        }

        function close(): string {
            if (root.dashPopoutLoader.item)
                root.dashPopoutLoader.item.dashVisible = false;
            return "THEME_PICKER_CLOSE_SUCCESS";
        }

        function toggle(): string {
            const dash = root.dashPopoutLoader.item;
            if (dash?.dashVisible && dash.currentTabId === "themes") {
                dash.dashVisible = false;
                return "THEME_PICKER_TOGGLE_SUCCESS";
            }
            return _openDashTab("themes", "all");
        }
    ...
    IpcHandler {
        function resolveTabIndex(tab: string): int {
            ...
            case "themes":
                return SettingsData.dashTabIndexForId("themes");
            case "wallpaper":
        ...
        function toggle(tab: string): string {
            if (root.dashPopoutLoader.item?.dashVisible) {
                root.dashPopoutLoader.item.dashVisible = false;
    ...
    IpcHandler {
        function getFocusedScreenName() {
            ...
            if ((CompositorService.isSway || CompositorService.isScroll || CompositorService.isMiracle) && i3Shim.workspaces?.values) {
    ...
    IpcHandler {
        function open(): string {
            PopoutService.openLauncher();
            return "LAUNCHER_OPEN_SUCCESS";
        }

        function close(): string {
            PopoutService.closeLauncher();
        ...
        function openWith(mode: string): string {
            ...
            PopoutService.openLauncherWithMode(mode);
    ...
    IpcHandler {
        function open(): string {
            if (root.changelogLoader.active && root.changelogLoader.item) {
                root.changelogLoader.item.show();
            } else {
                root.changelogLoader.active = true;
            }
            return "CHANGELOG_OPEN_SUCCESS";
        }

        function close(): string {
            if (root.changelogLoader.item)
                root.changelogLoader.item.visible = false;
            root.changelogLoader.active = false;
            return "CHANGELOG_CLOSE_SUCCESS";
        }
    ...
    IpcHandler {
        function currentWorkspaceName(): string {
            if (CompositorService.isHyprland)
                return Hyprland.focusedWorkspace?.name || String(Hyprland.focusedWorkspace?.id || "");
        ...
        function open(): string {
            ...
            if (root.workspaceRenameModalLoader.item) {
                root.workspaceRenameModalLoader.item.show(currentWorkspaceName());
    ...
    IpcHandler {
        function open(url: string): string {
            root.browserPickerModal.url = url && url.length > 0 ? url : "https://vanillagreen.com";
            root.browserPickerModal.open();
            return "BROWSER_PICKER_OPENED";
        }

        function close(): string {
            root.browserPickerModal.close();
            return "BROWSER_PICKER_CLOSED";
        }

        target: "browser-picker"
    }

    IpcHandler {
        function open(target: string, mimeType: string): string {
            root.appPickerModal.title = I18n.tr("Open with...");
            root.appPickerModal.targetData = target && target.length > 0 ? target : "file:///tmp/example.txt";
            root.appPickerModal.targetDataLabel = "file";
            root.appPickerModal.mimeType = mimeType || "";
            root.appPickerModal.rememberMimeTypes = [];
            root.appPickerModal.categoryFilter = [];
            root.appPickerModal.usageHistoryKey = "filePickerUsageHistory";
            root.appPickerModal.open();
            return "APP_PICKER_OPENED";
        }

        function close(): string {
            root.appPickerModal.close();
            return "APP_PICKER_CLOSED";
        }
