...
`publish-aur.yml` runs on two triggers, and each owns one package. The push on `main` owns `vgs-shell-git`, whose version counts the commits on that branch. That trigger cannot publish `vgs-shell`, whose source is a release tarball that exists only after the tag is cut, so the publisher defers it. The `workflow_call` from `release.yml` owns `vgs-shell` and runs once the tarballs are up. Both triggers select both packages, so a release run reaches `vgs-shell-git` as well. It finds the AUR already carrying a higher version whenever `main` advanced past the tag, and defers that package rather than failing the release.
