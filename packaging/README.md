...
now ships too little rather than too much. This is the authoritative statement of that figure —
other docs reference it rather than repeating the number. Derive it: `.totalSize` in
`themes/catalog.json` is the theme payload (regenerated by `scripts/gen-theme-catalog.py --write`,
kept fresh in CI by `scripts/check-package-assets.sh`); `du -sh themes config/vshell/icons` adds
the vendored icon themes. `scripts/check-package-assets.sh` fails if any in-repo
...
the download catalog plus every non-installed theme's screenshot (~23 MiB against the
