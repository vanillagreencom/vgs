...
`vshell.service` is `WantedBy=graphical-session.target`, and systemd never starts that target itself: a session target binds it and carries it up with the session, per [`systemd.special(7)`](https://www.freedesktop.org/software/systemd/man/latest/systemd.special.html). uwsm starts it, and Hyprland starts it from the release that ships its own `hyprland-session.target`, skipping it where uwsm already manages the session. VGS ships no producer of its own, because a second one would be a second session manager for a single session. A Hyprland launched straight from a TTY, without uwsm and on a release predating that target, produces neither, so the enabled unit never starts at the next login. `vshell setup` names that session rather than reporting a bare success, and gives the two ways out: uwsm, or `exec-once = vshell run` in the compositor config.
