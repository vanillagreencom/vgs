...
That same split governs the release ARTIFACTS, which is a separate question from what gets
installed. A release publishes a per-architecture core bundle (`vgs-<v>-linux-<arch>.tar.gz`,
~71 MiB) and one architecture-independent extras bundle (`vgs-<v>-assets.tar.gz`, ~1.0 GiB),
built by `scripts/build-release.sh` and `scripts/build-assets.sh`. Before that split every
download carried the extras — `install.sh` fetched ~1.1 GiB to install ~107 MiB, and so did
every distribution build — while the shell could already fetch individual themes on demand from
the pinned catalog. The extras bundle carries its own copy of `install-system.sh` so a recipe can
run `VGS_THEME_BUNDLE=extras` against it without the core tree.

The core bundle ships `themes/catalog-previews/` PRE-GENERATED, because it no longer contains the
themes those screenshots are of. `install-system.sh` copies them when present and falls back to
deriving them from the theme tree, which is what a source checkout still has. `check-release.sh`
asserts both halves: previews present in the core bundle, icons absent from it. This is the authoritative statement of that figure — other docs
...
`vgs-shell-assets` is its OWN AUR pkgbase rather than a split package of `vgs-shell`. makepkg
fetches a pkgbase's sources before it knows which split package was asked for, so while the two
shared one, `yay -S vgs-shell` downloaded the ~1.0 GiB of extras to install a shell that does not
use them. `vgs-shell-git` keeps its `vgs-shell-assets-git` split package: it builds from a git
checkout, which is one tree either way.
...
Maintainer recipes remain in `arch/`, `fedora/`, `debian/`, `ubuntu/`, `gentoo/`, and `void/`. Guix needs a packaged Quickshell dependency before VGS can publish a truthful channel. Release procedure: `project-skills/vgs-release/SKILL.md`.
