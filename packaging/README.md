...
`vshell.service` is `WantedBy=graphical-session.target`, and systemd never starts that target itself: a session target binds it and carries it up with the session, per [`systemd.special(7)`](https://www.freedesktop.org/software/systemd/man/latest/systemd.special.html). uwsm starts it, Hyprland starts it from the release that ships its own `hyprland-session.target`, and Niri starts it from its own `niri.service`, each skipping it where uwsm already manages the session. VGS ships no producer of its own, because a second one would be a second session manager for a single session. A compositor launched straight from a TTY, without uwsm or its own session unit, produces neither, so the enabled unit never starts at the next login. `vshell setup` names that session rather than reporting a bare success, and gives the two ways out: a session manager (uwsm, or Niri's own `niri-session`), or the compositor config's own startup line. `vshell.service` stays enabled under the compositor-config route too, so switching to a session manager afterwards needs `systemctl --user disable vshell.service` first.
