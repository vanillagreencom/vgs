...
## Validation
The suite is `scripts/validate [AREA]` (`go`, `qml`, `helper`, `packaging`,
`docs`, `all`) — run the area for what you touched. The command manifest and its
per-area scoping live in that runner; no command list is restated here, because
a partial copy reads as complete.
