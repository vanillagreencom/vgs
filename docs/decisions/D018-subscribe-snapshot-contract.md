...
**Accepted race**: a record that lands between subscribe's cached read and the push it appends leaves the older frame last on the wire for that service. The window is the few instructions between the two, and the service's next broadcast corrects it. Narrowing it would mean holding a lock across the push or dropping a frame by sequence, which costs more than the frame it saves.
