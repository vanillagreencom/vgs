...
**Verification**: `scripts/test-theme-apply-queue.js` executes `_runApply`, `_dispatchApply` and `_finishApply`, pinning that one apply reaches the helper at a time, that the rest follow in request order whether the one before them succeeded or was refused, that a launch the shell cannot start is answered where it fails so the queue empties rather than parks, and that a throwing completion handler still leaves the next apply running. `scripts/test-wallpaper-thumbs.py` pins that pruning keeps a live build's temp file and reclaims one whose process is gone. `scripts/test-switcher-scope.js` pins the per-monitor write, and `scripts/check-vshell-helper.py` the helper's write ordering.
