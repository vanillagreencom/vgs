...
- Four forms of VGS-owned difference are permitted. Outside them, a value the upstream publishes stays the upstream's, and a value it publishes nowhere is not invented. **Current state** below records the instances this decision has ruled on so far. A file or value it does not name is not yet ruled on, neither permitted nor pending, until a later pass names it.
  ...
  - A VGS value for a key the upstream answers nowhere in its own files, or answers only with a value `colors.toml` may not hold. The test is the role the key itself names: where the upstream paints that same role, on any surface, the key takes the value it carries there and is an upstream value rather than a VGS one, and `themes/THEMES-ATTRIBUTION.md` names the surface. A surface carrying a different role is no answer, however close its value sits. `synthwave84`'s `accent` is an upstream value under this test. `accent` names no terminal surface; its role is the theme's accent, and the extension paints that role `#f97e72` on nine workbench surfaces, so `accent` takes it. `synthwave84`'s `background` and `selection_background` are VGS values. `background` names the terminal's background, and the extension sets neither `terminal.background` nor the `panel.background` VS Code falls back to; `editor.background` and `sideBar.background` carry the editor and sidebar canvases, which are other roles, and the package already takes `editor.background` for `color0`. The extension's `terminal.selectionBackground` carries an alpha channel that `colors.toml` may not hold.
  ...
  - A curated app file for an app the upstream publishes no file for, where it improves the theme on that app. A community mapping is preferred over a generated render. The file is VGS's throughout: VGS fills the keys the mapping omits and owns the keys it sets. Such a file is not removed once written.
...
- The VS Code terminal-slot overwrite `augment_vscode_colors` performs, under the first form. It applies to every package this decision covers, not to one.
- `synthwave84`'s `background` and `selection_background`, under the second form.
...
- `synthwave84`'s `apps/btop.theme`, under the fourth form: the extension publishes no btop file, so the package carries the Omarchy import's mapping, with the five keys that held the old VGS accent set to the upstream accent.
