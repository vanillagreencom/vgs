...
- Four forms of VGS-owned difference are permitted. Outside them, a value the upstream publishes stays the upstream's, and a value it publishes nowhere is not invented. **Current state** below records the instances this decision has ruled on so far. A file or value it does not name is not yet ruled on, neither permitted nor pending, until a later pass names it.
  ...
  - A VGS value for a key or role the upstream sets no usable value for anywhere in its own files. Where the upstream sets no value for the key but paints the same role elsewhere, the key takes the value that role carries, and is an upstream value rather than a VGS one. `synthwave84`'s `background` and `selection_background` are VGS values under this form: the upstream sets no `terminal.background` and no `panel.background`, so it paints no terminal surface at all, and its `terminal.selectionBackground` carries an alpha channel that `colors.toml` may not hold. Its `accent` is not one: no terminal key answers the accent, and the upstream paints its accent role `#f97e72` on nine workbench surfaces, so `accent` takes that value.
