...
Theme imagery publishes to GitHub Releases on `vanillagreencom/vgs` under tags `themes-v1`, `themes-v2`, and so on. One asset per theme, named `vgs-theme-<name>-r<rev>.tar.gz`, holding that theme's complete package: `theme.json`, `colors.toml`, an optional `terminal-colors.toml`, `apps/*`, `backgrounds/` and `preview.png`. **AMENDED (2026-09-13):** an optional `ui-roles.toml` joins that list, the file a curated package states its own derived UI roles in. It is a definition file like the others: `CATALOG_ALLOWED_TOP_LEVEL` in `bin/vshell_helper.py` admits it, `catalog_relpaths` in `scripts/gen-theme-catalog.py` enumerates it, and `scripts/publish-theme-assets.py` packs whatever that returns, so an archive published without it would install a theme whose chrome the definitions no longer describe. `<rev>` is a per-theme integer that increases only when that archive's content changes, and it increases to the first revision the release does not already carry, which is not always by one.
...
- **The repository tree**, for everything the definitions carry: `name`, `mode`, `pair`, `source`, the 16 colours, `background`, `foreground`, `accent`, and a `files[]` entry with size and sha256 for each *definition* file that remains in the tree (`theme.json`, `colors.toml`, `terminal-colors.toml`, `apps/*`). **AMENDED (2026-09-13):** `ui-roles.toml` is a definition file too and is hashed with them, so editing a package's declared UI roles moves its `definitions` digest and `--check` fails until that theme's archive is republished, exactly as editing its `colors.toml` does. The count below is the one measured at decision time and is left as it stood. At decision time, 517 of the 1005 file entries were definition files, and they survive unchanged.
...
**"A second switch is offline and instant" means**: `vshell theme apply <name>` for an installed theme makes zero network calls. It reads `~/.config/vshell/themes/<name>/` and `/usr/lib/vshell/themes/<name>/` and nothing else, because `compose_theme_files()` in `bin/vshell_helper.py` composes from those two directories only. `theme catalog install` and `theme catalog update` are the only commands that reach the network.

### 5. Degradation, per path

| Path | Behaviour |
|---|---|
| `blueprint_from_wallpaper()` in `bin/vshell_helper.py` | Keeps raising. Its two callers, `theme extract-wallpaper` and `theme set-wallpaper --extract`, act on a path the user typed. A silent fallback there hides a typo. |
...
| Fastfetch fallback logo (`bin/vshell_helper.py`, `apply_fastfetch_logo_hook`) | Moves off `themes/coppernight/backgrounds/4-cats-anime.jpg` to `config/vshell/branding/fastfetch-logo.jpg`, a 600×600 crop of 18,856 bytes, so it no longer depends on a theme that may not be installed. |
...
The owner decided on 2026-09-13 that every theme's definitions ship in the base VGS package and a release archive carries only that theme's wallpapers. The marketplace browser is retired by VGS-321. A package update then refreshes every definition, and applying a theme needs no download first.

**Evidence**, measured at `ab553529`: `themes/` holds 81 theme packages. Their definitions total 3,761,038 bytes and the 480-pixel thumbnails 1,585,179 bytes, while `themes/catalog.json` reports a `totalSize` of 1,072,743,760 bytes over 81 archives. `packaging/install-system.sh` copied two of the 81 themes, so every other theme needed a download before first use.

This amendment supersedes these parts of the decision:

- **§1, archive contents.** An archive holds `backgrounds/*` and nothing else, and both earlier lists of definition files it carried no longer apply. `is_imagery` in `scripts/gen-theme-catalog.py` is the one owner of the split, and `CATALOG_ALLOWED_DIRS` in `bin/vshell_helper.py` admits `backgrounds/` alone, so a download refuses an archive member that is a definition or a preview.
- **§2, catalog inputs.** A catalog entry carries `name`, `mode`, `pair`, `source`, `size` and `assets`. The per-file `files[]` list, the colours and the lock's `definitions` digest are gone from the catalog file: the catalog listing reads a theme's colours from its shipped package, and nothing reads the file list or the digest. The lock is version 2, and generation refuses any other version, since an earlier lock pins archives that carry definitions. `scripts/gen-theme-catalog.py --package-files` lists what a theme package ships in the VGS package: every definition file and preview, and the default theme's wallpapers so a first boot has one. `scripts/check-package-assets.sh` compares the installed tree with that list.
- **§3, previews.** The 480-pixel thumbnail was sized on the catalog browser alone. Every theme now ships a full-size `themes/<name>/preview.jpg`, captured at the helper's `PREVIEW_SIZE` of 2560×1440 by `scripts/capture-theme-previews.py` and encoded as JPEG at quality 90. `scripts/capture-theme-previews.py --check` refuses a missing or smaller preview and a set over 64 MiB. The thumbnail stays, derived at publish time from the committed preview rather than from an archive. The earlier departure that took full-size previews out of the repository is reversed. VGS-321 retired the catalog browser, so the full-screen theme switcher is now the only consumer of the thumbnail, and it paints the thumbnail as a placeholder only while a theme's `preview` is empty: `theme list --json` reports `preview` and `thumbnail` apart. The `ThemeCatalogBrowser.qml` citations in the original §3 and its screenshot lookup no longer apply.
- **§4, installed location.** A download places its wallpapers in `~/.config/vshell/themes/<name>/` beside any user overlay, and never replaces a file it did not place. `theme catalog remove` deletes only the wallpapers the marker records, and `theme revert` keeps them while dropping the overlay. `theme catalog install` and `theme catalog update` are the commands that reach the network; `theme catalog updates` reads the shipped catalog offline. A download's marker names the wallpapers it placed and the sha256 each held, so an update replaces only the ones the user left alone, and only entries under `backgrounds/` count, so a directory an earlier release downloaded whole keeps its definitions as a user overlay: it reads as edited, `theme revert` drops them, and no step deletes them unasked. A marker with no file list does not identify a download.
...
- **§1, store home.** `themes-vN` releases publish to `vanillagreencom/vgs-themes`. That repository holds releases and their archives only, and no code, so no worktree or agent lane targets it. `ASSET_REPO` in `scripts/gen-theme-catalog.py` names it, and `scripts/publish-theme-assets.py` creates, reads and uploads to that repository through `gh release`. The releases page of `vanillagreencom/vgs` lists shell versions only, and a shell release is the one marked latest there. Archive naming, revisions, resume and the never-delete rule for published assets are unchanged.
- **§1, cadence.** `scripts/publish-theme-assets.py` refuses to open a new release while the newest release on `vanillagreencom/vgs-themes` is younger than `RELEASE_INTERVAL`, seven days. The refusal's first line is `release-too-recent <repo> <tag>`, and the run restores the lock it found. `--force` opens the release anyway. Uploading into a release that already exists, which is how an interrupted run resumes, is never refused.
- **§1, move between repositories.** The lock's `repo` field names the repository its pins live on. When it differs from `ASSET_REPO`, the publish script re-pins every theme, unpublished, to the first `themes-vN` number the asset repository does not carry, keeping each archive name and revision, and then uploads the batch as it uploads a dry run. The first publish after this amendment therefore uploads every archive to `themes-v1` on `vanillagreencom/vgs-themes`.
...
**The bundled default theme changes.** The owner's set is `roseofdune` (18 MB, `"mode": "light"`) and `bauhaus` (9.7 MB, `"mode": "dark"`), which excludes `coppernight` (19 MB), the current default. **`bauhaus` becomes the default**, the dark member of the pair, so the shell's first paint stays dark. The name is repointed in `config/vshell/settings.default.json`, `quickshell/vshell/Common/MethodTheme.qml`, `quickshell/vshell/Common/SettingsData.qml`, `quickshell/vshell/Common/settings/SettingsSpec.js`, `quickshell/vshell/Modules/Greetd/GreetdSettings.qml`, and `DEFAULT_THEME_NAME` in `bin/vshell_helper.py`, which is the helper's single spelling of it. The package's theme set becomes both bundled themes, so every install still has its default theme on disk.
