...
**"A second switch is offline and instant" means**: `vshell theme apply <name>` for an installed theme makes zero network calls. It reads `~/.config/vshell/themes/<name>/` and `/usr/lib/vshell/themes/<name>/` and nothing else, because `compose_theme_files()` in `bin/vshell-helper` composes from those two directories only. `theme catalog install` and `theme catalog update` are the only commands that reach the network.
