...
A publish creates the next `themes-vN` release and uploads only the archives whose content moved. A theme unchanged since `themes-v1` stays pinned to `themes-v1`. **No asset is ever deleted from a past `themes-vN` release**, so a checksum pinned by an older shell stays fetchable. An upload never replaces a name already on a release *with different bytes*; a name already there at the identical sha256 is an interrupted run resuming, not a collision.
...
`themes/thumbnails/<name>.jpg` ships in the repository and with the core package, which every install carries: 480 pixels wide, JPEG quality 82, derived from the full-size preview at publish time. Measured over the current 79 previews as generated by `scripts/publish-theme-assets.py`: 1,548,721 bytes in total.
