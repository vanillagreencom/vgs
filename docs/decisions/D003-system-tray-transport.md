...
`Quickshell.Services.SystemTray` host as the single owner of the
StatusNotifierItem (SNI) resource.
...
- `Quickshell.Services.SystemTray` runs inside the shell process and is the
  ...
  (`SystemTrayBar.qml:32`), and `VGSIPC.qml`'s `tray` handler
  (`list`/`activate`/`status`) reads the same collection.
- Menus for items that export a DBusMenu are built by VGS from `item.menu`,
  not by `display()`.
- `callContextMenuFallback(trayItemId, globalX, globalY)` is reached from three
  right-click paths (`SystemTrayBar.qml:92`, `:620`, `:1471`), all guarded by
  `!trayItem.hasMenu`. It runs `Quickshell.execDetached(["bash", "-c", …])` on
...
1. **It matches on a non-unique key.** SNI `Id` is application-unique, not
   ...
   (`SystemTrayBar.qml:41`) and the IPC handler both compose `id::tooltipTitle`
   precisely because `id` alone collides. The fallback matches `Id` and takes
   the first hit, so with two items from the same application the wrong one is
   asked to open its menu.
2. **It is a second enumerator of a resource the shell already owns.** The
   watcher registry is walked again, from a subprocess, while Quickshell's host
   holds a live subscription to it — the exact shape AGENTS.md § Backend rules
   forbids ("one owner per resource: never add a second watcher/daemon/poller
   for something the helper or QML already owns").
...
transport and the only owner of the SNI resource. The `ContextMenu` fallback
stays as it is, now documented as a known-lossy workaround for an upstream gap
rather than as an open question in a code comment.
...
| Second enumerator of the SNI registry | No new one | Yes | Yes |
| New surface to maintain and later delete | None | A `vshell` verb | A method + capability + QML gate |
...
already runs **CLI → shell** (`VGSIPC.qml:2003`). Routing a tray action
shell → CLI would make the shell shell out to a tool whose existing tray verbs
...
| **`vshell tray context-menu <id> <x> <y>` in `bin/vshell-helper`** (the TODO's second option) | Genuinely improves two things — the quadruple-escaped bash disappears from a QML string literal, and the code lands under `py_compile` + `scripts/check-vshell-helper.py` — but it keeps the double enumeration and the first-`Id`-wins misfire, adds a process spawn per right-click, and adds a CLI verb that exists only to be deleted when upstream lands. **Kept as the fallback plan** if the workaround has to live longer than expected. |
| **`tray.contextMenu` method on the Go backend** | The backend does own D-Bus-facing system services, but its only D-Bus bridge today is deliberately narrow: `backend/internal/services/dbusbridge/dbusbridge.go` allowlists exactly one system-bus signal (`login1 PrepareForSleep`) and rejects everything else. This would add a session-bus connection *and* an SNI registry walk — a second owner of the resource Quickshell's host already owns — plus a capability, an inventory entry and a QML gate, for one rarely-hit call that would still guess between same-`Id` items. |
