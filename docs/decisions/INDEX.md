...
| 2026-08-14 | D008 | — | Nested smoke seeds from shipped defaults; host `~/.config/vshell` is inert enrichment | A sandbox whose verdict depends on the machine it ran on is not a sandbox | An assertion genuinely needs host state, or the copy stops earning its keep | Active | [Full](D008-nested-sandbox-state-seeding.md) |
