...
- A catalog entry owns the command it declares and the file inside the package that command runs, and nothing else. An entry whose package exports more keeps its install off PATH with mise's `bin_path=` and names an `exec` path the stub and launcher run directly. `mise_export_conflicts` asks mise what each install exports and reports every undeclared executable that also answers elsewhere on PATH, on each stub refresh and so after every `vshell update run tools`. `test_an_install_may_not_export_a_command_the_entry_does_not_own` and `test_a_package_kept_off_path_runs_by_absolute_path` in `scripts/check-dev-tools.py` cover the rule and the absolute-path launch.
