...
- A catalog entry owns only the commands it declares, and an entry whose package exports more keeps the package's own directory off PATH. `vshell update run tools` and `vshell mise refresh` both run `mise_sync` and exit non-zero when an install exports an undeclared command that also answers elsewhere on PATH, or when mise cannot answer. [D016](../decisions/D016-a-catalog-entry-owns-only-the-commands-it-declares.md) records the mechanism, what the check covers and what it leaves out. `test_an_install_may_not_export_a_command_the_entry_does_not_own`, `test_a_package_kept_off_path_runs_by_absolute_path`, `test_the_refreshed_stub_and_the_launch_refusal_reach_the_owner` and `test_an_option_the_catalog_changed_reaches_an_earlier_install` in `scripts/check-dev-tools.py` cover it.
