...
- A catalog entry owns the command it declares, the file inside the package that command runs, and the executable its launcher names; nothing else. An entry whose package exports more keeps its install off PATH with an empty mise `bin_path=`, which moves the export to the install root, and names an `exec` path the stub and launcher run directly. `mise_export_check` asks mise what each installed agent, app and tool exports and fails when an undeclared executable also answers elsewhere on PATH, or when mise cannot answer; `vshell update run tools` and `vshell mise refresh` both exit non-zero on that outcome. An `envs` row has no package of its own and exports nothing to ask about. `mise_declare_options` re-declares an installed entry whose spec carries backend options, because mise records them once and `mise up` never revisits them. `test_an_install_may_not_export_a_command_the_entry_does_not_own`, `test_a_package_kept_off_path_runs_by_absolute_path` and `test_an_option_the_catalog_changed_reaches_an_earlier_install` in `scripts/check-dev-tools.py` cover the rule, the absolute-path launch and the migration.
