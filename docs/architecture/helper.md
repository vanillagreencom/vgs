...
- What a catalog install may put on PATH, and when VGS may change how a shell reaches an entry's command, is [D016](../decisions/D016-a-catalog-entry-owns-only-the-commands-it-declares.md); its Verification names the test for each rule.
