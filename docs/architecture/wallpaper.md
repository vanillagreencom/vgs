...
Covers: quickshell/vshell/Common/SessionData.qml, quickshell/vshell/Services/VGSThemeService.qml, quickshell/vshell/Modules/WallpaperBackground.qml, bin/vshell_wallpaper_thumbs.py
...
|---|---|---|
| Wallpaper switcher, Dash Wallpapers, Settings Themes | `VGSThemeService.setWallpaper` → `vshell theme set-wallpaper` | helper: package data and the `{wallpaper}` targets; shell: `SessionData` |
| Theme switcher, Dash Themes, Settings Themes, Icons tab, catalog download | `VGSThemeService.applyBlueprint` → `vshell theme apply` | the same, plus every other enabled target |
| Switcher "This monitor", Dash per-monitor buttons, Settings toggle, `wallpaper` IPC | `SessionData.setMonitorWallpaper` and `setPerMonitorWallpaper` | shell only: `session.json` |
