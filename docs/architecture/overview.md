...
- The supported Quickshell baseline is 0.3.1. Every packaging recipe whose format has a version slot requires it as a minimum, generated from `packaging/optional-packages.json`; `scripts/gen-package-metadata.py --check` fails when a recipe drifts from that file. A behavioural claim about the Quickshell binary names the version it was read on.
