...
- `lock` and `polkit` are exclusive: while one plugin holds one, another plugin naming it is not built, and it builds once the holder lets go. `PluginLogic.lendRefusal` decides it, `slotKey` and the bar's reconcile follow a copy of the holders taken after each change settles, and `scripts/test-plugin-logic.js` and the smoke pin it.
...
- The latency ceilings measure the whole shell. A per-plugin latency or memory budget is a target until a row measures it.
