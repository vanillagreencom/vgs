...
- A target writes one file from the applied theme unless its configuration says otherwise: `modes` renders its template once per listed mode into a destination that names the mode, and `modeVariants` adds `dark_<role>` and `light_<role>` so one file carries both. The second mode reads the theme's pair when one resolves, otherwise a mode transform of the applied palette; `mode_variant_blueprint` in `bin/vshell-helper` owns that choice. The agent CLIs that pick a variant from the terminal are the consumers.
- Selecting the generated theme inside another app's own settings file writes that one key and only when it differs, through `set_json_config_key` and `set_yaml_config_key` in `bin/vshell-helper`. The YAML writer is textual and refuses a block shape it cannot read, because VGS ships no YAML dependency.
