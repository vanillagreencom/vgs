...
- An uninstalled theme paints from `themes/thumbnails/<name>.jpg`, which ships with the package that every install carries. `theme_shipped_preview` in the helper is the one answer to what can be painted without rendering: the catalog browser reaches it through `catalog_entries`, and `theme list --json` through the same call with the package's own `preview.png`. Both resolve the thumbnail itself through `theme_thumbnail_path`.
