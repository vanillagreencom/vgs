...
- While that one-shot is unspent, the CLI status report names what will claim the name rather than the manual takeover command, because a takeover run by hand then races the shell's own and both write the undo record. `test_notification_status_withholds_the_manual_fix_before_the_first_run` in `scripts/check-vshell-helper.py` checks both sides.
