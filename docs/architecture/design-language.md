...
- On Hyprland the corners and border of every VGS window belong to the compositor: the rule `BlurService` installs matches the VGS window class, and `Widgets/VgsFloatingSurface.qml` follows its state through `compositorChrome`. Hyprland draws a dragged window's previous buffer in its new geometry, so client-painted chrome trails the edge by a frame while compositor chrome tracks it, and content is held one pixel off the edge the compositor repeats across a growing window. Layer surfaces keep drawing their own chrome; Hyprland's layer rules accept neither `rounding` nor `border_size`, and a layer surface is never dragged to resize. Without the rule, a window paints its own chrome with the border off its outermost pixel (`borderInset`). See the window border check in `scripts/qml-smoke.sh`.
