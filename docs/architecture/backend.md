...
- A subscribe snapshot is a cached read. `RegisterSnapshot` returns last-known-good state without blocking; a service whose state comes from an external command registers that query with `RegisterSnapshotRefresh`, which subscribe kicks onto the service's own goroutine through `backend/internal/refresh`. A repeat subscribe re-sends only the services the connection did not already hold. See `backend/internal/server/` and its tests.
- Each connection owns a bounded outbound queue drained by one writer goroutine. Events coalesce per service, so a slow reader receives the latest state instead of a backlog, and a connection past the bound is dropped. `Broadcast` never waits on a peer. See `backend/internal/server/` and its tests.
- A method whose queue is full answers `busy:` instead of blocking the connection's read loop. An idempotent setter registered with `RegisterLatest` keeps one waiting call and answers the replaced one with `superseded:`. See `backend/internal/server/` and its tests.
