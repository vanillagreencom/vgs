...
| tmux | `~/.config/tmux/vgs-theme.conf` or dotfiles-linked target; the hook sources it into every RUNNING tmux server it finds under `$TMUX_TMPDIR`, `/run/user/<uid>` and the temp directory. A server started with its own `TMUX_TMPDIR` listens nowhere the shell's environment names, and a bare `tmux source-file` would start an empty server on the default socket and report success, so each socket is probed with `list-sessions` first |
...
`SessionData.setWallpaper/setWallpaperColor/clearWallpaper/setModeWallpaper` propagate to every monitor when per-monitor mode is on — the global path is not displayed in that mode, so without propagation theme applies and browser clicks would change nothing visible. Single-screen assignment stays explicit via `setMonitorWallpaper` (the per-monitor buttons in the wallpaper browser). `setPerMonitorWallpaper(enabled)` SEEDS on the off-to-on edge (`_seedPerMonitorFromCurrent`) before it flips the flag, so every enable is seeded by construction — switcher, the dash's per-monitor buttons, the Settings toggle and the `wallpaper` IPC handler's `setFor` alike. Unseeded, the flag alone republishes whatever an earlier per-monitor session left retained, and every screen holding an entry jumps the instant the mode goes on. The seed writes what each connected screen currently shows into the four maps that flag gates: `monitorWallpapers`; `monitorWallpapersLight`/`Dark` when `perModeWallpaper` is on (the current mode from what the screen SHOWS, since cycling writes `wallpaperPath` without the mode paths, the other from its own global); and `monitorWallpaperFillModes`, which would otherwise re-crop a screen to a retained `Fit`/`Stretch`. Retained per-screen cycling is forced OFF at the same edge — `WallpaperCyclingService` makes `cyclingActive` true on `perMonitorWallpaper` alone, so a retained `enabled: true` would start a slideshow on a screen nobody named and overwrite the wallpaper just seeded. Product calls, recorded in D010: the Settings toggle is seeded too (turning per-monitor wallpapers back on must NOT restore months-old assignments), and an enable leaves nothing moving or re-cropped on a screen the caller did not name.
...
A rail of angled, overlapping shots with the selection opened out in the middle; arrow keys and the scroll wheel page, Enter applies, Esc cancels — over a filter the first Esc clears THAT, so a mistyped term does not cost the browse. Additive: the dash tabs and Settings pickers are untouched, and paging applies nothing — the desktop changes only on Enter.
...
| Enter | by scope: all monitors `VGSThemeService.setWallpaper`, this monitor `SessionData.setMonitorWallpaper` behind the seeded enable — either unless an apply is in flight | `VGSThemeService.applyBlueprint` unless an apply is in flight |

- `Modals/Switcher/FullScreenSwitcher.qml` owns the surface, paging, filter and keys; each switcher maps its data to `{image, label, key}` and handles `applied`. The look is Omarchy 4's image picker, reproduced in `SwitcherCarousel.qml`: overlapping parallelogram slices (`SwitcherSlice.qml` masks and outlines one) with the selection opened out over a 50% scrim — ONE scrim: the modal's own backdrop is set to zero opacity, or `modalDarkenBackground` composited a second one and the dim depended on a user setting — no header, counter, input box, footer, and no window border (`enableBorder: false` on `VgsModal`, or a full-bleed surface traces one around the screen). Omarchy's proportions are logical pixels grown by `unit`, NOT by a property named `scale`, which is `Item`'s own transform. Slivers decode to a fixed 1536x864: covering a tall narrow crop from a 16:9 source needs a decode several times its display width, so a rail of display-size decodes costs hundreds of MB. The selected slot is `SwitcherStage.qml` — visible image plus prev/next lookahead, all `cache: true`. The lookahead only hits under the SAME pixmap-cache key as the visible image, which folds in `fillMode` as well as `sourceSize` — `decodeWidth`/`decodeHeight`/`decodeFillMode` are declared once and bound by both for that reason. The held selection is a KEY, not an index, so a filter does not cost the user their place — and `holdCurrent` only ever writes one: a query matching nothing empties the list, and clearing there would lose it mid-keystroke. Slivers read a pre-sized JPEG from `~/.cache/vshell/wallpaper-thumbs/` (`theme wallpaper-thumbs --all`, keyed on path+size+mtime; a miss falls back to the source): 13 decode in ~21 ms not ~596 ms, and the SELECTED slot still reads the original, so full-size quality is unchanged. Not the dash grid's 256x256 PNG `imagecache` — that size degrades the rail, and PNG is 3x the bytes here. Residency is those three pixmaps plus a hysteresis band of `slicesPerSide + 2` slivers each side — kept a little past the visible edge so paging back does not re-decode, and RELEASED beyond it. Latching a sliver's source on for good (Omarchy's shape, over a dozen entries) retains one pixmap per entry a long browse ever passed; 79 installed themes is 79 of them.
...
- Enter is gated on `VGSThemeService.applyInFlight` (just `applyBlueprint`/`setWallpaper`), NOT on `busy`: `busy` counts every non-background command, so it blocks on an unrelated restyle started from a settings tab and does not see the background reads the switcher itself fires. A blocked Enter keeps the switcher up and says so in the caption under the rail; the message clears on `onCanApplyChanged`, with the 2.5 s timer only as an upper bound. On a multi-monitor setup a semi-transparent pill above the rail chooses Enter's scope (VGS-212): **All monitors** — the default on every open, the service path above — or **This monitor**, a `SessionData.setMonitorWallpaper` write on `effectiveScreen` that turns `perMonitorWallpaper` ON when it is off (the product decision: no bounce to Settings) through the one seeded setter above — which is what keeps the pick off every OTHER monitor — checks the screen still exists FIRST, reads the write back and toasts a miss — SessionData refuses a write with only a warn, and a silent single-screen path is the failure mode the service path exists to avoid. The gate covers BOTH routes: a service apply succeeding late propagates its wallpaper to every monitor and would silently overwrite a single-screen assignment made while it ran.
- The wheel pages through `step` like every key, so it cannot answer an empty pager differently or skip the intent latch; `wheelSteps` carries the leftover between events, because a touchpad sends fractions of a notch and truncating each one drops them all. The base re-seeds the selection from `activeKey` on every list and every `activeKey` change until the user moves it (`userMoved`, set by every paging key and Home/End through the one `navigate` adapter, whose `latchesIntent` refuses an EMPTY pager — or the first open's seed would never land — and unconditionally by `updateFilter`, the ONE place the filter is written now that there is no input box; cleared on open). Both `show()` paths dispatch their read and `open()` in the same tick, so the list at open is by construction the previous one; latching on data arrival instead left the switcher on a stale set after a theme change. Once the user has paged, nothing re-seeds. The base's own per-open reset runs from a self-targeted `Connections`, so a subclass `onOpened:` coexists with it instead of replacing it. While the scope pill is up (`scopeToggle` non-null: wallpaper switcher, more than one screen) Tab and Backtab flip it instead of paging — taken off paging deliberately; arrows, Home/End and the wheel still page — and the flip does NOT latch `userMoved`, so the seed follows the chosen scope: "this monitor" seeds that screen's wallpaper, "all monitors" seeds only a unanimous one (`scopeSeedKey` answers "" under disagreement — no single entry is "current"). The scope itself resets to all-monitors on every open.
- Failures are reported as failures. `blueprintsLoadFailed` / `wallpapersLoadFailed` (each with its own `*LoadError` detail — `lastError` is a shared slot every command overwrites) mark a failed helper call so an empty list is not asserted as "you have none"; a list already on screen is KEPT browsable and carries a `staleNotice` caption instead. The wallpaper wording is `VGSThemeService.wallpapersStaleNotice` — one property, shown by both the switcher banner and the dash Wallpapers tab, and it NAMES the theme the retained set belongs to when that is no longer the active one (`applyBlueprint` re-reads on success, so a failed re-read leaves the previous theme's set on screen). A failed apply toasts from the switcher via `Modals/Switcher/ThemeApplyReporter.qml`, correlated by the request id `applyFinished` carries — from a keybind no settings tab is loaded to report it, and `applyCompleted` alone is emitted by 19 unrelated operations. The request id is minted per CALL (`<label>#<seq>`): `setWallpaper` names every wallpaper alike, so two overlapping calls shared a key. Applies run UNCOALESCED too: `_runApply` passes an EMPTY Proc id, so Proc mints a random self-cleaning one. Proc's debouncer folds only same-tick calls, so inferring that a newer request had replaced an unlaunched one dropped that live token and left its failure unreported. `setWallpaper` also claims `selectedWallpaper` optimistically and records the claiming REQUEST in `_wallpaperSlotOwner`, which the rollback and the success-path `SessionData.setWallpaper` test and `_finishApply`/`clearWallpaper` release; keyed on the PATH, a mid-apply `refreshCurrent` falsified it and a SUCCESSFUL apply skipped `SessionData` and never reached the monitors. Success is silent, because an open Themes tab already info-toasts the same completion; a FAILURE with that tab open is toasted twice, which is the accepted cost of the switcher reporting a failure nothing else would when it is driven from a keybind.
- Two ways in. A keybind: `Common/KeybindActions.js` ships open/close/toggle rows per switcher, and `SUPER+SHIFT+W` / `SUPER+SHIFT+T` are the SUGGESTED chords — VGS ships no compositor binds at all, so a fresh install has neither until someone sets one. And a `SwitcherShortcutRow` on the wallpaper and theme settings pages, which binds that page's own switcher through `KeybindsService`. Settings' **Browse** buttons stay on the DASH tabs: from Settings the full picker is the more capable surface (folders, per-monitor assignment, downloads), and the switcher is the keyboard-driven one — capture is `Widgets/KeybindCaptureField.qml`, lifted out of `KeybindItem.qml` so a chord is decoded in ONE place, `ShortcutInhibitor` and the Alt+Shift ghost-event workaround included. The row says so when `vgsBindsIncluded` is false and offers the same Setup the keybinds tab does: a bind written to a file the compositor config never includes is a shortcut that silently goes nowhere. A chord another action already owns is held back behind Replace, because `keybinds set` deletes every existing entry for a key before appending — saving straight through takes the other shortcut away without saying so.
- Covered by `switcher_check` in `scripts/qml-smoke.sh` with `modalDarkenBackground` on and off: the two OPEN paths (`open` and `toggle`-to-open) assert the MEASURED layer geometry covers the output; `close`, `toggle`-to-close and Escape assert the surface unmaps. Escape needs `wtype`, and the verdict line says so when it is missing. The switcher list is DISCOVERED by ROOT ELEMENT (`FullScreenSwitcher`, indented or not, brace on either line) across `quickshell/vshell`, not by filename: a new switcher lives in `Modals/Switcher/` and declares a `vshell:`-prefixed `layerNamespace` whose derived IPC target `VGSIPC.qml` registers; one the check cannot place fails the phase. That whole phase is behind `--nested`, which CI never passes, so the pure selection logic (`wrapIndex`, `clampIndex`, `seedIndex`, `shouldReseed`, `enterOutcome`, `latchesIntent`, `navIndex`) is marked off in `FullScreenSwitcher.qml` and executed by `scripts/test-switcher-selection.js`, which does run in CI. The wallpaper subclass's scope arithmetic (`scopeChoiceExists`, `scopeSeedKey`, `applyRoute`) is marked off the same way in `WallpaperSwitcherModal.qml` and executed by `scripts/test-switcher-scope.js`, which also pins the Tab claim, the verified single-screen write, the seeded enable (all four maps and the cycling force-off, on the off-to-on edge, before the flag flip) and that the theme switcher never grows the toggle.
