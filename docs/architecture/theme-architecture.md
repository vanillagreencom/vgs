...
- `Modals/Switcher/FullScreenSwitcher.qml` owns layout, paging, filter and keys; each switcher maps its data to `{image, label, badge, key}` and handles `applied`. `SwitcherStage.qml` is the preview area: visible image plus prev/next lookahead, all `cache: true`. The lookahead only hits if it decodes under the SAME pixmap-cache key as the visible image, which folds in `fillMode` as well as `sourceSize` — `decodeWidth`/`decodeHeight`/`decodeFillMode` are declared once and bound by both for that reason. Residency is bounded to the three referenced pixmaps.
