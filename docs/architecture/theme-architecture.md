...
- The manifest is generated by `scripts/gen-theme-catalog.py --write` and
  ...
  `{path, size, sha256}`, plus a pinned `source.ref` (a release tag) that the
  download URLs are built from. `scripts/check-package-assets.sh` fails when it
  is stale: a stale manifest makes every download of a changed theme fail its
  checksum.
...
- Downloads are ordinary user theme packages under
  ...
  and marked with a `.vgs-catalog.json` file. That marker is what makes
  `catalog remove` safe: without it a directory is a hand-made user theme and
  removal is refused, as it is for the currently applied theme.
