...
- Three ways in. A keybind: `Common/KeybindActions.js` ships open/close/toggle rows per switcher, and `SUPER+SHIFT+W` / `SUPER+SHIFT+T` are the SUGGESTED chords — VGS ships no compositor binds at all, so a fresh install has neither until someone sets one. The Settings **Browse Wallpapers** / **Browse Themes** buttons, which open the switcher rather than the dash tab they used to — Settings has no shell-root scope, so `VGS.qml` registers each modal with `ModalManager.registerSwitcher` and the buttons call `showSwitcher`, falling back to the dash when nothing is registered rather than appearing to work. And a `SwitcherShortcutRow` on each of those two pages, which binds that page's own switcher through `KeybindsService` — capture is `Widgets/KeybindCaptureField.qml`, lifted out of `KeybindItem.qml` so a chord is decoded in ONE place, `ShortcutInhibitor` and the Alt+Shift ghost-event workaround included. The row says so when `vgsBindsIncluded` is false and offers the same Setup the keybinds tab does: a bind written to a file the compositor config never includes is a shortcut that silently goes nowhere. A chord another action already owns is held back behind Replace, because `keybinds set` deletes every existing entry for a key before appending — saving straight through takes the other shortcut away without saying so.
