...
`SessionData.setWallpaper/setWallpaperColor/clearWallpaper/setModeWallpaper` propagate to every monitor when per-monitor mode is on — the global path is not displayed in that mode, so without propagation theme applies and browser clicks would change nothing visible. Single-screen assignment stays explicit via `setMonitorWallpaper` (the per-monitor buttons in the wallpaper browser). Turning the mode ON is `SessionData.enablePerMonitorWallpaperFromCurrent()`, NOT a bare `setPerMonitorWallpaper(true)`: the flag alone republishes whatever an earlier per-monitor session left in `monitorWallpapers` (and, under per-mode, in the light/dark maps `syncWallpaperForCurrentMode` refills it from), so every screen holding a retained entry jumps to an old wallpaper the instant the mode goes on. The seeded enable writes each connected screen's current picture into all three maps first — the current mode's from what the screen SHOWS, since cycling writes `wallpaperPath` without the mode paths — so the flip is invisible and only the caller's own write afterwards moves anything. The dash's per-monitor buttons, the Settings toggle and `wallpaper set-for` still flip the flag bare (VGS-214).
