...
- The base re-seeds the selection from `activeKey` on every list and every `activeKey` change until the user moves it (`userMoved`, set by paging, Home/End and filter typing, cleared on open). Both `show()` paths dispatch their read and `open()` in the same tick, so the list at open is by construction the previous one; latching on data arrival instead left the switcher on a stale set after a theme change. Once the user has paged, nothing re-seeds. The base's own per-open reset runs from a self-targeted `Connections`, so a subclass `onOpened:` coexists with it instead of replacing it.
- Failures are reported as failures. `blueprintsLoadFailed` / `wallpapersLoadFailed` (each with its own `*LoadError` detail — `lastError` is a shared slot every command overwrites) mark a failed helper call so an empty list is not asserted as "you have none"; a list already on screen is KEPT browsable and carries a `staleNotice` banner instead. A failed apply toasts from the switcher via `Modals/Switcher/ThemeApplyReporter.qml`, correlated by the request id `applyFinished` carries — from a keybind no settings tab is loaded to report it, and `applyCompleted` alone is emitted by ~25 unrelated operations. Success is silent, because an open Themes tab already info-toasts the same completion; a FAILURE with that tab open is toasted twice, which is the accepted cost of the switcher reporting a failure nothing else would when it is driven from a keybind.
