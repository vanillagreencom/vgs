...
- The same singleton owns the toplevel and monitor refresh fan-out from Hyprland raw events; other components keep their own `rawEvent` connections for their own concerns. One list of event names per refresh it drives, neither list carrying both names of a v1/v2 pair. On Hyprland the fan-out and the `ToplevelManager` list share one zero-interval timer, so one user action costs one rebuild; the two `NiriService` handlers rebuild directly and are not coalesced. `scripts/test-compositor-events.js` checks both lists, the timers and the two coalesced producers.
