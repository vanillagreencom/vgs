...
- `Modules/WorkspaceOverlays/OverviewWidget.qml` owns the overview's own toplevel, workspace and monitor fetches for as long as it is on screen, because `HyprlandToplevel` carries no geometry or class property and every tile's rectangle, icon and click target comes from `lastIpcObject`, which only an explicit fetch fills. That is the other half of the split above: the fan-out issues no toplevel fetch. The overview builds one widget per screen, so one action asks for those fetches once per monitor; each stays one round trip because `HyprlandIpc` drops a second request while the first is outstanding. `scripts/test-overview-refetch.js` checks the single fetch function, its producers and the reads that depend on it.
