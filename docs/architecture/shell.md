...
- The same singleton owns the toplevel and monitor refresh fan-out from Hyprland raw events; other components keep their own `rawEvent` connections for their own concerns. One list of event names per refresh it drives, neither list carrying both names of a v1/v2 pair, and both producers of the toplevel view share one zero-interval timer so one user action costs one rebuild. `scripts/test-compositor-events.js` checks both lists and the coalescing.
