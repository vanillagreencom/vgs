...
`[agent-skills]` in `kendex.toml` are generated and rewritten by
`kendex refresh` from upstream defaults, so findings about the values there
...
`kendex.settings.toml` is public by design: non-secret project defaults,
