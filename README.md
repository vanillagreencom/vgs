...
The enabled unit starts with `graphical-session.target`, which your session manager brings up: uwsm does, and Hyprland does from the release that ships its own `hyprland-session.target`. A Hyprland launched straight from a TTY without either starts neither, so the unit never runs at your next login, and `vshell setup` says so when it sees that session. Launch Hyprland through uwsm, from the `hyprland-uwsm.desktop` session entry or with `uwsm start hyprland`, or start VGS from your compositor config with `exec-once = vshell run` instead.
