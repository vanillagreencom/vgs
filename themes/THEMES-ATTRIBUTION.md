...
| `synthwave84` | [RobbOwen.synthwave-vscode 0.1.20](https://open-vsx.org/extension/RobbOwen/synthwave-vscode/0.1.20), its `terminal.*` and `terminalCursor.foreground` keys plus the surfaces named below | `apps/vscode-theme.json`: the theme file in the same package. `apps/btop.theme`, `apps/neovim.lua` and the `vim-synthwave84` tree it names are VGS's, for apps this upstream publishes no file for | MIT, © Robb Owen |
