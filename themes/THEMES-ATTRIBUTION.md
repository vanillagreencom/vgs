...
VGS maps these palettes into `colors.toml` and derives app files from its own render targets. Light ANSI colors derive from the bright syntax and UI colors. Contrast adjustments use the helper's OKLCH lightness mapping with hue preserved. The dark blue selection and bright-black band use the same adjustment. No upstream VS Code theme file is included.
