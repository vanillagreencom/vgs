...
VGS maps these palettes into `colors.toml` and derives app files from its own render targets. Light terminal body and band values live in `terminal-colors.toml` so editors retain a light background and dark text. Light ANSI colors derive from the bright syntax and UI colors. Contrast adjustments use the helper's OKLCH lightness mapping with hue preserved. The dark blue selection and bright-black band use the same adjustment. Curated app adjustments set readable btop selection text and light Obsidian tag colors. No upstream VS Code theme file is included.
