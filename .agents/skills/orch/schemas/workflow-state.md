...
| `pr` | object | Pull-request size state. `baseline_lines` and `baseline_origin` initialize as null and do not authorize fix rounds. `size_check` is the verdict `branch-size-check` records at launch, fix-round mint, cut acceptance and submission: `base_sha` and `head_sha`, the commits it compared; `production_lines`, `test_lines` and `mirror_lines`, the added lines in each part; `production_allowance` and `test_allowance` from the issue's `**Expected delta**` line, null where absent; the `verdict` (`pass`, `production_over`, `tests_over`, `allowance_missing`) and its `reason`. Every check overwrites the record. A reader binds it to a head by `head_sha`; review-gate's `pr-watch.sh` annotates its `disarmed` line with the record for the PR head and reports any other as stale |
