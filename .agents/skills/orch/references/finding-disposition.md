...
Size allowance: `branch-size-check` judges added production and test lines against the issue's `**Expected delta**` line at launch and before the push ([`../workflows/submit-pr.md`](../workflows/submit-pr.md) § 1.2). `dev-round-write` uses the same verdict before fix delegation. Exit 3 stops the item-by-item fix path; report the over-allowance classes, counts and allowances, then cut back to the Done-when. Close every thread on cut code with the deleting sha. The cut is a round like any other, stamped `dev-round-write --cut`; acceptance refuses it unless the branch came back within the issue allowance.
