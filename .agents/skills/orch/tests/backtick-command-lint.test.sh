#!/usr/bin/env bash
# Regression lint for kendex#721 (same command-shape class as #714's
...
# pattern) IS flagged.
if [[ -n "$(scan_backtick "$(inject_block offender 'rg -n "`kendex refresh`" skills/')")" ]]; then
...
# flagged.
if [[ -z "$(scan_backtick "$(inject_block safe "rg -n '\\x60kendex refresh\\x60' skills/")")" ]]; then
...
if [[ -z "$(scan_backtick "$(inject_block comment '# never run rg -n "`kendex refresh`"')")" ]]; then
...
printf '\nNever search for `kendex refresh` with a literal backtick.\n' >> "$PROSE_SCRATCH"
...
printf '\n```markdown\nUse `kendex refresh` after merging.\n```\n' >> "$NONCMD_SCRATCH"
