...
# (kendex #634, coverage follow-up #640).
#
...
rcG=0
outG="$(run_validate CC-950 --include-children-of CC-950 --container 2>&1 >/dev/null)" || rcG=$?

assert_ne "G: a (one PR) bundle is refused under --container" "$rcG" 0
assert_contains "G: the refusal names the (one PR) marker" "$outG" "(one PR)"
...
assert_ne "F: leaf --container (no children in cache) exits nonzero" "$rcF1" 0
