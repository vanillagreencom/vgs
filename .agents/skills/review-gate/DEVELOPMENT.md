...
There is no dedicated settings key and no disposition protocol, and while enforcement is on nothing disables the term; the only switch that reaches it is `REVIEW_GATE_MODE = "off"`, which answers approved for the whole gate without reading any evidence. Nothing written in the pull request clears the verdict; it clears when the commit the gate relies on carries no such block, normally a fresh review at a new head. The known limit is the errored-attestation filter's mirror: a body quoting the heading at the start of a line counts as a real block.
