...
- The `pull_request_target` job never executes PR-controlled code. Every checkout pins a shallow copy of the default branch with stored credentials disabled and refuses an empty default-branch resolution rather than falling back. When docs classification runs, the predicate completes the shallow history for the base SHA and the head at `refs/pull/<number>/head`. It verifies both commit objects and their common ancestor, and it never checks out or executes PR files.
