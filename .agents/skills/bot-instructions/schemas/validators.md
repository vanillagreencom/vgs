...
**`[bot-instructions.repo] code_review_path` is the second such entry, and it is outside that table.** Its clauses are shapes rather than content refusals — the marker path class, a `.md` suffix, no empty, `.` or `..` component, the `CODE_REVIEW_TREE` parent, no `AGENTS.md` basename in any case, and no upper-case basename — and `config._code_review_path` encodes them. `repo-toml.md` § `[bot-instructions.repo]` states them on the key's own row. Six clauses, eight controls in `toml-schema.test.sh`: the parent-directory clause carries two, a sibling directory and a directory nested below the tree, and the `AGENTS.md` clause carries two, one spelling in each case.
