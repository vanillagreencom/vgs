...
export SIZE_RATCHET_CLASSES='*=1k'
run --staged
...
sed 's/if \[ "\$n" -gt "\$limit" \]; then/if false \&\& [ "$n" -gt "$limit" ]; then/' "$SR" >"$TMP/mutant/size-ratchet"
if cmp -s "$SR" "$TMP/mutant/size-ratchet"; then
  printf 'mutation did not change the comparison\n' >&2
  exit 1
fi
chmod +x "$TMP/mutant/size-ratchet"
SR="$TMP/mutant/size-ratchet"
run --staged
CONTROL_RC=0
