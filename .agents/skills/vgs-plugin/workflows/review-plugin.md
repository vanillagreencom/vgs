...
2. Open every entry point and confirm: `property var shell: null` exists, `moduleName` equals the manifest id, every timer, `Process`, `FileView` and connection sits inside the entry point's tree, no capability is read from `bar`, and no copy of `shell.settings` is held past the assignment.
