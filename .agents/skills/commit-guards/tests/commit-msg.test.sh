...
run_stdin() { # MESSAGE [env-assignment] — feed via stdin; sets OUT and RC
  ...
  RC=0
  OUT="$(cd "$R" && printf '%s\n' "$1" |
    env ${NO_SETTINGS:+COMMIT_GUARDS_SETTINGS_FILE=/dev/null} ${2:+"$2"} "$CM" 2>&1)" || RC=$?
}
...
expect_pass 'release: cut 2.6.6' "custom type list admits its types" "COMMIT_GUARDS_COMMIT_TYPES=feat release"
expect_fail 'fix: no longer a type' "custom type list rejects everything else (control)" "COMMIT_GUARDS_COMMIT_TYPES=feat release"
run_stdin 'feat: x' "COMMIT_GUARDS_COMMIT_TYPES=Feat"
[ "$RC" -eq 2 ] && ok "a non-lowercase type entry is exit 2" || bad "bad type entry is exit 2" "rc=$RC out=$OUT"
run_stdin 'feat: x' "COMMIT_GUARDS_COMMIT_TYPES= "
[ "$RC" -eq 2 ] && ok "an empty type list is exit 2" || bad "empty type list is exit 2" "rc=$RC out=$OUT"

NO_SETTINGS=""
echo "=== settings file resolution ==="
printf '[env]\nCOMMIT_GUARDS_COMMIT_TYPES = "docs"\n' >"$R/kendex.settings.toml"
...
printf 'COMMIT_GUARDS_COMMIT_TYPES=docs\n' >"$R/.env"
...
printf 'COMMIT_GUARDS_COMMIT_TYPES=chore\n' >"$R/.env.local"
run_stdin 'feat: base type'
...
run_stdin 'feat: base type' "COMMIT_GUARDS_SETTINGS_FILE=/dev/null"
...
printf '[env]\nCOMMIT_GUARDS_COMMIT_TYPES = "docs feat"\n' >"$RI/kendex.settings.toml"
...
OUT=""; RC=0
OUT="$(cd "$RI" && printf 'feat: not in the outside list\n' | COMMIT_GUARDS_SETTINGS_FILE="sub/../../outside-settings.toml" "$CM" 2>&1)" || RC=$?
...
expect_pass "fix(KEN-1): $(rep x 61)" "a raised cap admits the same header" "COMMIT_GUARDS_SUBJECT_MAX=100"
expect_fail "fix(KEN-1): $(rep x 20)" "a lowered cap refuses a header the default admits" "COMMIT_GUARDS_SUBJECT_MAX=20"
run_stdin 'fix: x' "COMMIT_GUARDS_SUBJECT_MAX=0"
...
run_stdin "$(rep q 90)" "COMMIT_GUARDS_SUBJECT_MAX=20"
[ "$RC" -eq 1 ] \
...
run_stdin "fix: a subject with ${ESC}[31m in it" "COMMIT_GUARDS_SUBJECT_MAX=5"
[ "$RC" -eq 1 ] && case "$OUT" in *"$ESC"*) false ;; *) true ;; esac \
...
case "$OUT" in *"CHANGELOG.md counts only under COMMIT_GUARDS_CHANGELOG_COLLATE=1"*) ok "the record is named as the release commit's own write, and what declares it" ;;
...
OUT=""; RC=0
OUT="$(cd "$RC_REPO" && printf 'chore(release): collate the changelog\n' | COMMIT_GUARDS_CHANGELOG_COLLATE=1 "$CM" 2>&1)" || RC=$?
[ "$RC" -eq 0 ] && ok "COMMIT_GUARDS_CHANGELOG_COLLATE=1 makes the collated record the entry" \
  || bad "COMMIT_GUARDS_CHANGELOG_COLLATE=1 makes the collated record the entry" "rc=$RC out=$OUT"
...
OUT=""; RC=0
OUT="$(cd "$RC_REPO" && printf 'fix(KEN-3): change a crate\n' | COMMIT_GUARDS_CHANGELOG_REQUIRED_PATHS= "$CM" 2>&1)" || RC=$?
...
OUT=""; RC=0
OUT="$(cd "$RC_REPO" && printf 'fix(KEN-3): change a crate\n' | COMMIT_GUARDS_CHANGELOG_REQUIRED_PATHS=/etc/crates "$CM" 2>&1)" || RC=$?
...
OUT=""; RC=0
OUT="$(cd "$RC_REPO" && printf 'fix(KEN-3): change a crate\n' | COMMIT_GUARDS_CHANGELOG_REQUIRED_PATHS="crates/* /etc/crates" "$CM" 2>&1)" || RC=$?
[ "$RC" -eq 2 ] && case "$OUT" in *"must be repo-root-relative"*) true ;; *) false ;; esac \
...
git -C "$RC_REPO" add -A
OUT=""; RC=0
OUT="$(cd "$RC_REPO" && printf 'fix(KEN-5): change a crate\n' | COMMIT_GUARDS_CHANGELOG_COLLATE=1 COMMIT_GUARDS_CHANGELOG_RECORD="docs/My Changelog.md" "$CM" 2>&1)" || RC=$?
...
rm -rf -- "${RC_REPO:?}/docs"
OUT=""; RC=0
OUT="$(cd "$RC_REPO" && printf 'fix(KEN-3): change a crate\n' | COMMIT_GUARDS_CHANGELOG_PATHS="" "$CM" 2>&1)" || RC=$?
[ "$RC" -eq 2 ] && case "$OUT" in *"COMMIT_GUARDS_CHANGELOG_PATHS names no path"*) true ;; *) false ;; esac \
...
OUT=""; RC=0
OUT="$(cd "$RC_REPO" && printf 'fix(KEN-3): change a crate\n' | COMMIT_GUARDS_CHANGELOG_RECORD=changelog.d/fixed/x.md "$CM" 2>&1)" || RC=$?
