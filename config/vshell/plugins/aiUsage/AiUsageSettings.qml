...
PluginSettings {
    ...
    StyledRect {
        ...
        Column {
            ...
            StyledText {
                width: parent.width
                text: "Backed by the 'ai-usage' helper (in ~/.local/bin), which wraps the claudebar/codexbar engines.\n\n• Claude: log in with the 'claude' CLI (reads ~/.claude/.credentials.json)\n• Codex: log in with 'codex login' (reads ~/.codex)\n\nExtra accounts are picked up automatically: any config directory holding its own login (the ones your CLAUDE_CONFIG_DIR / CODEX_HOME wrappers point at) is listed separately, labelled by its signed-in email. Profiles whose tokens live in the desktop keyring can't be polled and are left out.\n\nThe bar pill keeps one slot per provider, each with its own icon and in a fixed order, so a number never changes meaning. A slot shows that provider's usage %, colored by the worst active window across the accounts on screen; an exclamation mark when that provider answered and the answer was unusable — not signed in, or the usage API failed; an ellipsis while its first fetch is still running; and a dash when the provider is fine but there is nothing left to show, which is every one of its accounts hidden. Click it for session, weekly and per-model limits with reset countdowns; with several accounts you get a per-account overview, and clicking a row expands it."
                font.pixelSize: Theme.fontSizeSmall
                color: Theme.surfaceVariantText
                wrapMode: Text.WordWrap
                lineHeight: 1.4
